...
- [x] content/topics/utility-theory.mdx(ReaderGuide + L0-2フル + L3-6 planned。サンクトペテルブルクの期待金額発散を「定数項だけの数列の和」、log₂効用の収束を高校数学の「等差×等比数列の和(ずらして引く)」で導出、リスクプレミアムの非負性をイェンセンの不等式の直感(弦が凹関数の下にある高校数学の«上に凸»の定義)から証明。decision-analysisへ前提リンク)
