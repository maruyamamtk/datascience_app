...
### コンテンツ拡充 #84 — [J-5] モデル選択基準（model-selection-criteria）
- [x] 作業ブランチ feat/84-model-selection-criteria をworktree内で作成
- [x] 出典確認: SPEC §3 J-5「情報量規準、AIC、BIC、Mallows の Cp、cross validation」。Bookmemoに専用章が見当たらないため一般的な統計学の教科書的説明で構成(タスク指示どおり出典タグなし)。estimation-methods.ts/multiple-regression.ts(olsFit)を型として精読し重複回避
- [x] 計算層 lib/stats/model-selection-criteria.ts(「真に効くのはx1・x2だけ、x3〜x5は無関係」な固定回帰データ(n=24、決定的LCG)に対し説明変数0〜5個のネストモデルを当てはめ、濃縮対数尤度→AIC・BIC・Mallows Cp・k分割交差検証誤差を計算。olsFit(multiple-regression.ts)を再利用し回帰の当てはめ自体は再実装しない)+ Vitest 31(seed探索でAIC≠BICとなる具体例(seed=20017)を実測で選定、決め打ちしない教訓を遵守)
- [x] 状態層 lib/store/model-selection-criteria.ts(createTopicStore×1: controls={selectedK, foldCount}、ステッパーは1つだけなのでメインストアのframeを共用、tasks/lessons.md #76の判断目安)
- [x] 描画層 CriteriaLab(L0, モデルサイズ選択→RSS/AIC/BIC/Cp表+Δ比較折れ線グラフ+3本の数式が強連動)/ CriteriaStepper(L1, ネストモデルを1つずつコマ送りで当てはめ表を組み立て)/ PenaltyLab(L1, 標本サイズnスライダー→AIC罰則(2)とBIC罰則(log n)の比較、e²≈7.39の交点)/ CvLab(L2, 分割数選択→交差検証MSE曲線とAIC/BIC/CVの選択結果比較)/ ModelSelectionQuiz(5問) + frames.ts + Vitest 7
- [x] 用語ノード4件新規作成(information-criterion・aic・bic・mallows-cp、likelihood-function/maximum-likelihood/overfitting/cross-validation/degrees-of-freedom/adjusted-r-squared/residual-sum-of-squaresは既存を再利用し新規作成しなかった)
- [x] content/topics/model-selection-criteria.mdx(ReaderGuide + L0-2フル + L3-6 planned、正規誤差OLSの対数尤度をσ²の最尤推定まで進めた「濃縮対数尤度」の式を高校数学レベルの代数で導出、BICの罰則がAICより大きくなる境界n=e²≈7.39を対数・指数の関係で導出、Mallows CpのC_p≈pという読み方を基準モデル自身のCp=p_fullという恒等式から導出、estimation-methods/learning-frameworkへ前提リンク)
- [x] 検証(vitest 1332 / tsc / lint / build / Playwright実機)
- [x] backlog ✅done、todo レビュー追記

#### レビュー: モデル選択基準トピック（2026-07-22）
- **変更概要**: J群（モデル評価指標）5本目・前提 estimation-methods, learning-framework。「真に効くのはx1・x2だけ(x3・x4・x5は無関係なダミー変数)」という固定回帰データ(n=24件、決定的LCG生成)に対し、説明変数を1個ずつ増やした6個のネストモデルを比較する。CriteriaLab(L0)はモデルサイズボタンを押すとRSS・対数尤度・AIC・BIC・Cpの表とΔ比較折れ線グラフ、3本の数式(AIC/BIC/Cp)が実時間で更新される。実装のなかでseedを実測探索し(tasks/lessons.md「最適パラメータを決め打ちしない」の教訓を遵守)、AICが無関係な変数x3を含むk=3を最良と誤って判断する一方、BIC・Cp・交差検証はいずれも正しく真のモデルk=2を選ぶ、という教科書的な現象を実際に再現する具体例(seed=20017)を採用した。CriteriaStepper(L1)はこの6モデルをコマ送りで組み立て、PenaltyLab(L1)は標本サイズnスライダーでAICの罰則(定数2)とBICの罰則(log n)の交点n=e²≈7.39を数式・グラフと連動させて確認できる。CvLab(L2)はk分割交差検証(2/3/4/6/8/12分割を選択可能)でモデルサイズを比較し、情報量規準とは全く別ルートでも同じ結論(k=2)にたどり着くことを示した。
- **検証結果**: Vitest **1332 passed**（+38: model-selection-criteria計算層31・frames 7、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ）。build成功(398ページ、新規`/topics/model-selection-criteria`・4件の新規`/terms/*`すべて生成)。`pnpm dev` + Playwright実機確認: CriteriaLabでk=2ボタンをクリックし別呼び出しでcriteria-row-2のハイライト・aria-pressed=trueへの更新を確認、term-aic_val/term-aic_kの表示値(87.4/3)が計算層の期待値と一致しBIC/Cpのkatex-term--activeクラスがtrue・AICはfalseになる(k=2はBIC/Cpの最良だがAICの最良はk=3のため)ことを確認、CriteriaStepperの「1コマ後へ」でフレームが1/8→2/8に進み「▶再生」ボタンで3秒待って別呼び出しで2/8→7/8まで自動進行したことを確認(教訓#80の再発なし)、PenaltyLabのスライダーをn=30に動かしterm-pen_logn=3.401への更新を確認、CvLabの「8分割」ボタンクリックでaria-pressed=true・term-cv_val=2.211への更新とCV/BIC/AICの選択k比較(k=2/k=2/k=3)を確認、4件の新規用語ページ(/terms/aic等)すべて200・用語ページ内の相互リンク(BIC→/terms/bic等)のクリック遷移を確認・コンソールエラー0件、太字崩れ0件(CommonMarkフランキング規則の機械的スキャナで確認)。モバイル幅(390px)でLevel0ラボの表・グラフ・数式が視覚的に崩れないことをスクリーンショットで確認したが、`document.documentElement.scrollWidth`(503px)が viewport(390px)を上回りページ全体が横スクロール可能な状態を検出した——同じ現象を既存の多クラス分類トピック(scrollWidth 1049px)でも確認し、KaTeXのkatex-mathml(スクリーンリーダー用の隠し要素)がposition:absoluteのままscrollWidth計算に寄与する、サイト全体に既存する軽微な問題と判断し本Issueのスコープ外とした(視覚的な崩れ・要素の欠落は無い)。
- **設計判断 / つまずき**: **「argminを決め打ちしない」教訓の徹底** — 当初は適当なseedでデータセットを作ったが、AIC・BIC・Cp・交差検証がすべて同じモデルサイズを選んでしまい「BICの方が小さいモデルを選びやすい」という教科書的な主張を具体的な数値で見せられなかった。使い捨てスクリプトで300個のseedを走査し、実際にAIC≠BICとなる具体例を実測で選定した(tasks/lessons.md「最適パラメータを決め打ちしない」の教訓をモデル選択という新しい文脈でも実践)。**Cp・AIC・BICのパラメータ数kの数え方を1つに統一** — Mallows Cpの古典的な定義ではσ²の推定を別扱いすることが多く、AIC/BICの「k=切片+係数+σ²」という数え方と食い違いやすい(数式の結論だけ流用すると定数倍・+1のズレが混入しやすい、tasks/lessons.md #75と同種の落とし穴)。本トピックではタスク指示にある「AIC=−2×対数尤度+2k(kはパラメータ数)」という定義に忠実に従い、k=paramCount=切片+説明変数の数、とAIC・BIC・Cpの3指標で共通の数え方に統一し、フルモデル自身のCp=p_fullという恒等式のテストで数え方の一貫性を検証した。**cross-validation・likelihood-function・maximum-likelihood・overfitting・degrees-of-freedom・adjusted-r-squared・residual-sum-of-squaresを新規作成せず既存用語を再利用** — 事前にcontent/terms/を確認し、モデル選択に必要な周辺概念のほとんどが既存トピック(estimation-methods, learning-framework, multiple-regression)で既に用語ノード化されていたため、新規作成はinformation-criterion・aic・bic・mallows-cpの4件に絞った。**J-5完了**、進捗 58/82。次は K-1 ベイズ統計の基礎（前提: probability-basics, distribution-characteristics）へ。

### コンテンツ拡充 #85 — [K-1] ベイズ統計の基礎（bayesian-basics）
- [x] 作業ブランチ feat/85-bayesian-basics をworktree内で作成
- [x] 出典精読: Bookmemo『Pythonでスラスラわかるベイズ推論「超」入門』第3章「ベイズ推論とは」（事前分布・尤度関数・事後分布の3概念、事後∝尤度×事前、試行回数の影響、事前分布の種類、頻度主義との違い）。既存 content/terms/bayes-theorem.mdx・prior-probability.mdx・posterior-probability.mdx・beta-distribution.mdx・naive-bayes.mdx を精読し、離散事象の確率(prior/posterior-probability)と連続パラメータθの分布(本トピックで新設するprior/posterior-distribution)を明確に区別
- [x] 計算層 lib/stats/bayesian-basics.ts(コイン投げ10回の固定小データセットで事前分布→尤度→事後分布の逐次更新、ベータ-二項共役updateBetaWithCounts、ベイズファクターbayesFactor10(ベータ-二項周辺尤度/点仮説尤度、lnGamma/betaPdfは continuous.ts を再利用)、Jeffreys/Lee&Wagenmakersスケールでの強さ分類interpretBayesFactor、頻度論の両側p値binomialTwoSidedPValue(対比用)、ベイズ判別posteriorProbClass1・decision boundary探索findDecisionBoundaries(gaussianPdfはnaive-bayes-knn.tsを再利用))+ Vitest 27
- [x] 状態層 lib/store/bayesian-basics.ts(createTopicStore×1: controls={priorAlpha, priorBeta, sensitivityWeight, bfN, bfK, bfTheta0, discPrior1, discX}、ステッパーは1つ(PosteriorUpdateLab)だけなのでメインストアのframeを共用、tasks/lessons.md #76の判断目安)
- [x] 描画層 PosteriorUpdateLab(L0, コイン投げを1回ずつ観測するたびに事前分布(灰)→事後分布(青)のベータ曲線がコマ送りで更新される核心的ステッパー、事前分布α₀・β₀スライダーと数式Beta(α₀+k,β₀+m)=Beta(?,?)が強連動)/ PriorSensitivityLab(L1, 弱い/強い/選択した事前分布3本の事後平均推移を比較する感度ラボ)/ BayesFactorLab(L2, n・k・θ₀スライダー→ベイズファクターのJeffreysスケールバー・数式・頻度論p値との対比)/ BayesianDiscriminantLab(L2, 事前確率×尤度の2曲線と決定境界、事後確率に基づく判別)/ BayesianBasicsQuiz(5問) + frames.ts + Vitest 5
- [x] 用語ノード6件新規作成(prior-distribution・posterior-distribution・conjugate-prior・bayes-factor・bayesian-hypothesis-testing・bayesian-discriminant)、既存のprior-probability/posterior-probability/beta-distribution/naive-bayes/likelihood-ratio-test/p-valueへ相互リンクを追加(離散事象の確率と連続パラメータの分布の違いを本文で明示)
- [x] content/topics/bayesian-basics.mdx(ReaderGuide + L0-2フル + L3-6 planned、ベイズの定理を「事象」から「パラメータの分布」へ拡張する導出、ベータ-二項共役の更新式を指数法則だけで導く高校数学レベルの橋渡し、ベータ-二項周辺尤度(エビデンス)をベータ関数の定義から導く、probability-basics/distribution-characteristicsへ前提リンク、L3=ベイズ線形回帰(regularization-sparseと橋渡し)・L4=CausalImpact・L5=MCMC(K-2)・L6=階層ベイズ(K-3)をplanned枠で明示)
- [x] 検証(vitest 1364 / tsc / lint / build / Playwright実機)
- [x] backlog ✅done、todo レビュー追記

#### レビュー: ベイズ統計の基礎トピック（2026-07-22）
- **変更概要**: K群（ベイズ統計）1本目・前提 probability-basics, distribution-characteristics。「コインを1枚ずつ投げて表が出る確率θを推定する」という最小の具体例で、事前分布(灰)→事後分布(青)のベータ曲線が観測のたびにコマ送りで更新される様子をPosteriorUpdateLab(L0)で示した。最初の3投を意図的に「表・表・表」と続く固定データにすることで、序盤は事前分布の«歯止め»が効き平均が0.5から動きにくいこと、観測が増えるほど尤度の影響が事前分布を上回っていくことを体感できるようにした。PriorSensitivityLab(L1)は弱い事前分布Beta(1,1)・強い事前分布Beta(20,20)・選択した事前分布Beta(w,w)の3本の事後平均推移を重ねて描き、観測数が増えるほど3本が収束する様子を示す。BayesFactorLab(L2)は点仮説θ=θ₀と複合仮説θ~Beta(1,1)のベイズファクターBF₁₀をJeffreys/Lee&Wagenmakersスケールのバーで可視化し、同じデータに対する頻度論の両側p値の結論(有意/非有意)と並べて対比できるようにした。BayesianDiscriminantLab(L2)は1特徴量2クラスの「事前確率×尤度」曲線を描き、その交点が決定境界になること、事前確率を動かすと境界が移動して同じ観測xでも判定が変わりうることを示した。
- **検証結果**: Vitest **1364 passed**（+32: bayesian-basics計算層27・frames 5、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ）。build成功(405ページ、新規`/topics/bayesian-basics`・6件の新規`/terms/*`すべて生成)。`pnpm dev` + Playwright実機確認: PosteriorUpdateLabの「1コマ後へ」を3回押し別呼び出しでstep-counter(4/11)・coin要素(表/表/表が開示・残りは?のまま)・term-k/term-m/term-alphaPost/term-betaPost(3/0/4/1)への更新を確認、「▶再生」ボタンを押し実際にフレームが最終(11/11)まで自動進行してから1ティック遅れてボタンラベルが「再生」に戻ることを確認(教訓#80の再発なし)、事前分布αスライダーを10に動かし別呼び出しでterm-alpha0=10・事前分布曲線のpoints変化を確認、PriorSensitivityLabの重みスライダーをw=25に動かしラベル表示の追従を確認、BayesFactorLabでn=20→8にスライダーを動かすとkが15→8へ自動クランプされ(bf-k-slider.max=8と一致)、ベイズ的結論(H1を強い証拠で支持)と頻度論のp値結論(有意)が並んで表示されることを確認、BayesianDiscriminantLabのクラス1事前確率スライダーを0.5→0.9に動かすと事後確率が51.0%→90.4%に変化し決定境界のSVG座標が実際に移動することを確認、6件の新規用語ページ(/terms/bayes-factor等)すべて200・相互リンク(→bayesian-hypothesis-testing)のクリック遷移を確認・コンソールエラー0件、太字崩れ0件(CommonMarkフランキング規則の機械的スキャナで9ファイルをスキャンし0件)。モバイル幅(390px)でLevel0〜2すべてのラボが視覚的に崩れないことをスクリーンショットで確認したが、`document.documentElement.scrollWidth`(529px、差139px)がviewportを上回りページ全体が横スクロール可能な状態を検出した——既存の`model-selection-criteria`(差113px)と同程度でありKaTeXの`.katex-mathml`起因のサイト全体の既知の問題(tasks/lessons.md #84)と判断し本Issueのスコープ外とした。
- **設計判断 / つまずき**: **決定境界探索のグリッドサーチが格子点ちょうどで交点を検出すると重複カウントするバグをテストで発見** — findDecisionBoundariesを等分散2クラス・事前確率0.5の対称なケースでテストしたところ、真の交点(x=5.5)がちょうどグリッド分割点と一致し、`diff=0`の前後2点でそれぞれ「符号が変わった」と誤検出され境界が2件返る(期待は1件)というテスト失敗を発見した。原因は`Math.sign(0)=0`が前後どちらの符号とも異なると判定されるため。対策として、検出した根が直前の根と近すぎる(格子間隔の半分未満)場合は重複として無視するガードを追加し解消した——「小さな具体例で原理を見せる」ときほど数値がきれいに割り切れてこの種の境界条件バグが顕在化しやすい、という教訓。**離散事象の確率(prior/posterior-probability)と連続パラメータの分布(prior/posterior-distribution)を明確に区別** — probability-basicsで既に「事前確率」「事後確率」という用語ノードが存在したため、本トピックの「事前分布」「事後分布」をそのまま同名の別concept扱いにすると混乱する。既存2用語のaliasesから重複する「事前分布」「事後分布」を削除し、本文に「対象が1事象からパラメータ空間全体の分布へ一般化されたもの」という橋渡しの一文を追加することで、用語間の関係を誤魔化さずに示した。**乱数生成を使わずコイン投げ10回を直接書き下す** — 他トピックはLCGで決定的な合成データを生成するのが定石だが、本トピックは「小さな具体例(数個のデータ点)」の教育効果を最大化するため固定配列`[1,1,1,0,1,0,0,1,0,0]`を直接定義した——序盤3連続«表»→終盤5勝5敗という物語を意図的に選べ、LCGのシード探索も不要になった(SSR/CSRハイドレーション不一致のリスクも最初から存在しない)。**K-1完了**、進捗 59/82。次は K-2 ベイズ計算法（MCMC）（前提: bayesian-basics, monte-carlo-methods）へ。

### コンテンツ拡充 #86 — [K-2] ベイズ計算法（MCMC）（mcmc-methods）
- [x] worktree内で作業ブランチ feat/86-mcmc-methods を作成
- [x] 出典精読: Bookmemo『Pythonでスラスラわかるベイズ推論「超」入門』第4章「はじめてのベイズ推論実習」（PyMCによるサンプリング一連の流れ、tune/draws、ArviZのplot_trace/plot_posterior/summary、divergence問題、収束診断r_hat≤1.01・ess_bulk≥400・mcse_mean≤0.01の目安）。前提2トピック(bayesian-basics/monte-carlo-methods)を精読し、既に説明済みの「事後分布の考え方」「乱数で試して数える発想」を重複させず参照に留めた
- [x] 計算層 lib/stats/mcmc-methods.ts(決定的LCG相当のmulberry32+Irwin-Hall近似ガウスscaledGaussian、Metropolis-Hastingsの1ステップ/チェーン生成(metropolisHastingsStep/Chain, betaTargetKernel)、ギブスサンプリングの半ステップ/チェーン生成(gibbsChain, 2変量正規分布の条件付き分布gibbsConditionalSd)、収束診断(splitRhat=分割R-hat, effectiveSampleSize=自己相関からのESS近似、time-series.tsのacfを再利用)) + Vitest 30(受理判定が正規化定数の有無に依らず完全一致することを直接証明するテストを含む)
- [x] 状態層 lib/store/mcmc-methods.ts(createTopicStore×3: メインストアはcontrols={proposalSd, gibbsRho}のみでframeを持たず、MHステッパー・Gibbsステッパーの2つがそれぞれ専用の空controlsストアのframeを使う、tasks/lessons.md #76の判断目安通り)
- [x] 描画層 MHStepper(L0, K-1と同じBeta(6,6)を正規化定数抜きで目標にしたMetropolis-Hastingsのランダムウォークを1ステップずつコマ送り、受理=緑/棄却=赤の色分けと訪れた状態のヒストグラムが目標曲線に近づく様子)/ GibbsStepper(L1, 2変量正規分布のx更新(紫,横移動)⇄y更新(緑,縦移動)を交互にコマ送り、常に受理されることを対比)/ ConvergenceLab(L2, 提案幅σを共有する400ステップの長いチェーンでトレースプロット・ヒストグラム・分割R-hat・ESS・受理率が即座に追従)/ McmcMethodsQuiz(5問) + mh-frames.ts/gibbs-frames.ts + Vitest 11
- [x] 用語ノード4件新規作成(metropolis-hastings・gibbs-sampling・r-hat・effective-sample-size)、既存mcmc.mdxのseeAlso/aliasesを整理(「メトロポリス」「ギブスサンプリング」の重複aliasを新設ノード側へ寄せ、リンクで接続)
- [x] content/topics/mcmc-methods.mdx(ReaderGuide + L0-2フル + L3-6 planned、Metropolis-Hastingsの受理確率r=min(1,π(θ')/π(θ))が詳細釣り合いを満たすことを場合分けだけ(微分不要)で示す導出、詳細釣り合い⟹定常分布を総和の入れ替えだけで示す導出、分割R-hatを分散分析の群間/群内分散の類推で導く導出、有効サンプルサイズを分散の展開Var(ΣXi)=ΣVar+2ΣΣCovから導く導出、bayesian-basics/monte-carlo-methods/markov-chainsへ前提リンク、L3=HMC/NUTS・L4=CausalImpact・L5=変分ベイズ/逐次モンテカルロ・L6=階層ベイズ(K-3)をplanned枠で明示)
- [x] 検証(vitest 1405 / tsc / lint / build / Playwright実機)
- [x] backlog ✅done、todo レビュー追記

#### レビュー: ベイズ計算法（MCMC）トピック（2026-07-25）
- **変更概要**: K群2本目・前提 bayesian-basics, monte-carlo-methods。K-1で共役事前分布により解析的に求めたBeta(6,6)事後分布を「正規化定数を知らないふり」をして、今回はMCMCでサンプリング近似するという発展的な立ち位置を明確にした。MHStepper(L0)はMetropolis-Hastings法の「現在地→提案→受理/棄却→次の現在地」を1ステップずつコマ送りし、受理は緑・棄却は赤で色分け、これまで訪れた状態のヒストグラム(水色)が目標分布の曲線(灰色)へ近づいていく様子を同時に見せる。GibbsStepper(L1)は2変量正規分布(相関ρ)を目標に、yを固定してxを更新(横移動,紫)→xを固定してyを更新(縦移動,緑)を交互にコマ送りし、Metropolis-Hastingsと違い棄却が存在しない(常に受理)ことを対比した。ConvergenceLab(L2)は提案分布の標準偏差σ(L0のステッパーと同じcontrolsを共有)を400ステップの長いチェーンに適用し、トレースプロット・ヒストグラム・分割R-hat・有効サンプルサイズ(ESS)・受理率が即座に追従する様子を見せ、σが小さすぎると受理率は上がるがESSが下がり(自己相関が強くなる)ことを実測で確認できるようにした。
- **検証結果**: Vitest **1405 passed**（+41: mcmc-methods計算層30・mh-frames 5・gibbs-frames 6、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ、修正済みexhaustive-deps警告1件）。build成功(410ページ、新規`/topics/mcmc-methods`・4件の新規`/terms/*`すべて生成)。`pnpm dev` + Playwright実機確認: MHStepperのフレームスライダーを1〜12へ順に送り（別呼び出しでの読み取りに徹し教訓#31を再発させず）、受理(緑#16a34a)5回・棄却(赤#dc2626)7回の混在を確認、term-pCur/pProp/ratio/uが各ステップの実際の値(例: r=0.01, u=0.641で棄却)と一致することを確認、ヒストグラムバーの高さがステップごとに変化することを確認。GibbsStepperの「1コマ後へ」でx更新(紫#7c3aed)→y更新(緑#0d9488)の色切り替えと、tex文字列を切り替える設計(教訓#79の推奨パターン)によりterm-condVal/condMean/drawnが両更新で正しく切り替わることを確認。ConvergenceLabのσスライダーを0.3→0.06に動かすと受理率47%→85%・R-hat 0.998→1.24・ESS 89→7に変化し、σを小さくすると受理率は上がるが実質的な情報量(ESS)は下がるという教育的ポイントを実測で確認、L0のσスライダー表示にも同じ値が反映され2つのLevelでcontrolsが共有されていることを確認。モバイル幅(390px)でL0〜L2すべてのコンポーネントが視覚的に崩れないことをスクリーンショットで確認（`document.documentElement.scrollWidth`差142pxは既存トピック(bayesian-basics差139px)と同程度でKaTeX起因の既知の問題(tasks/lessons.md #84)と判断）。コンソールエラー・警告0件。太字崩れ: CommonMarkフランキング規則の機械的スキャナで新規6ファイルをスキャンし3件発見(「版**NUTS(...)**は」「する**逐次モンテカルロ(...)**は」「size)**による」——いずれも閉じ`**`の直後が仮名という教訓#73のパターン)、閉じ`**`の直後にスペースを挿入して修正し再スキャンで0件を確認。
- **設計判断 / つまずき**: **K-1と同じBeta(6,6)事後分布を「正規化定数を知らないふり」で目標にする設計** — MCMCの実務上の価値は共役事前分布が使えない一般のモデルで発揮されるが、最初からそういう例で学ぶと「本当に正しく近似できているか」を確かめる手段がない。既に答えを知っている題材で、正規化定数抜きの核(betaTargetKernel)を使っても正規化済みのbetaPdfを使っても受理判定が完全に一致することを直接テストで証明し(lib/stats/mcmc-methods.test.ts)、ヒストグラムが理論曲線に収束する様子で「MCMCが本当に正しい分布を復元できているか」を目視確認できるようにした。**Gibbsステッパーの数式は«tex文字列の切り替え»で実装(教訓#79を踏襲)** — x更新/y更新で数式の構造(XとYの主客が入れ替わる)が変わるため、当初は条件付きで別々の`<MathFormula>`をマウント切替する誘惑があったが、教訓#79「条件付きマウント切替は直後の1回だけ更新が効かない」を踏まえ、`<MathFormula>`は常時1つだけマウントしたまま`tex`プロパティの文字列だけを`updated`に応じて切り替える設計にした。Playwrightで実際に切替直後の1回目から数式が正しく更新されることを確認し、教訓の再発がないことを検証した。**MHStepperの色分けはpropose/resolveを1フレームに統合** — 当初は「提案」「受理/棄却の決定」を別々の2フレームに分ける設計も検討したが、既存トピック(PosteriorUpdateLabなど)が「1回の観測=1フレーム」という粒度を採用していることに合わせ、1ステップ(提案+受理/棄却の決定)を1フレームにまとめてCalloutで一体的に説明する設計にした——コード・フレーム数(13)ともシンプルになった。**分割R-hatを分散分析の群間/群内分散の類推で導出** — R-hatの公式をいきなり天下り式に示すのではなく、既存の[分散分析](/topics/analysis-of-variance)トピックで学んだ「群間分散B・群内分散W」の枠組みをチェーンの前半/後半という2群に適用する形で導出し、既存トピックとの橋渡しを明示した。**太字崩れの機械的スキャナを流用** — 教訓#73/#78で確立したCommonMarkフランキング規則のスキャナスクリプトを本トピックの新規6ファイルに対しても実行し、3件の「閉じ`**`の直後が仮名」パターンを機械的に検出・修正した——目視だけでは見落としていた可能性が高く、スキャナの再利用価値を再確認した。**K-2完了**、進捗 60/82。次は K-3 階層ベイズモデル（前提: mcmc-methods）へ。

### コンテンツ拡充 #87 — [K-3] 階層ベイズモデル（hierarchical-bayes）
- [x] worktree内で作業ブランチ feat/87-hierarchical-bayes を作成
- [x] 出典精読: Bookmemo『Pythonでスラスラわかるベイズ推論「超」入門』第5章「ベイズ推論プログラミング」（データ分布のベイズ推論、線形回帰のベイズ推論、階層ベイズモデル(複数店舗/地域の効果測定、pm.ConstantData/pm.Deterministic)、潜在変数モデル(IRT・混合分布)）。前提2トピック(bayesian-basics/mcmc-methods)を精読し、bayesian-basics L3「ベイズ線形回帰は到達途上」・mcmc-methods L6「階層ベイズはK-3で扱う」という前方参照を引き継ぐ形で範囲分担を設計、重複を避けた
- [x] 計算層 lib/stats/hierarchical-bayes.ts(決定的mulberry32+Irwin-Hall近似scaledGaussianで6クラス(生徒数5〜40人と段階的に増加)のテスト平均点データを生成、プールなし/完全プール/階層ベイズ(部分プーリング)の3推定量比較(compareEstimates)、精度加重平均の核となる汎用関数precisionWeightedMean(階層ベイズの事後平均とベイズ線形回帰の傾き事後平均の両方がこの1関数に帰着する設計)、縮小の重みshrinkageWeight、説明変数を中心化して切片・傾きを直交させるベイズ線形回帰bayesianLinearRegression(閉形式でRidge回帰のMAP推定と同型)、2成分混合正規分布の事後所属確率responsibility(潜在変数モデル入門)) + Vitest 38
- [x] 状態層 lib/store/hierarchical-bayes.ts(createTopicStore×1: controls={tau2, priorVarSlope, latentX}、ステッパーはページ全体で1つ(ShrinkageLab)だけなのでメインストアのframeを共用、tasks/lessons.md #76の判断目安通り。initialFrameCountをグループ数で固定)
- [x] 描画層 ShrinkageLab(L0, τ²スライダー+6クラスをサンプルサイズ昇順にコマ送りするStepPlayerを同一ストアで統合、灰=プールなし・黒破線=完全プール・青=階層ベイズの3推定値を数直線上に並べ、現在フレームの行をハイライト)/ BayesianRegressionLab(L1, n=8点の小標本回帰にOLS(灰破線)とベイズ事後平均(青線)を重ね、傾きの事前分散τ_β²スライダーでRidge同型の縮小を体感)/ LatentMixtureLab(L2, 2成分混合正規分布の密度曲線+観測値xスライダーで事後所属確率responsibilityが連動)/ HierarchicalBayesQuiz(5問) + frames.ts(ShrinkageStepper用) + Vitest 5
- [x] 用語ノード5件新規作成(hierarchical-bayesian-model・partial-pooling・shrinkage-estimation・bayesian-linear-regression・latent-variable-model、相互seeAlsoで接続し既存のconjugate-prior/prior-distribution/posterior-distribution/ridge-regression/bayes-theorem/k-meansへリンク)
- [x] content/topics/hierarchical-bayes.mdx(ReaderGuide + L0-2フル + L3-6 planned、事後平均が«精度加重平均»になることを平方完成(高校数学の2次関数の最小値)だけで導出しベイズ線形回帰の傾き縮小(Ridge回帰のMAP推定と同型、λ=σ²/τ_β²)へ橋渡し、mcmc-methodsへ前提リンク・bayesian-basics/mcmc-methods側のK-3前方参照(planned枠)を実装済みの本トピックへのリンクに更新)
- [x] 検証(vitest 1448 / tsc / lint / build / Playwright実機)
- [x] backlog ✅done、todo レビュー追記

#### レビュー: 階層ベイズモデルトピック（2026-07-25）
- **変更概要**: K群3本目・前提 mcmc-methods（間接的にbayesian-basics）。6クラスのテスト平均点(生徒数がA組5人〜F組40人と段階的に異なる)を題材に、プールなし(自クラス平均そのまま)・完全プール(全クラス共通の単一平均)・階層ベイズ(部分プーリング、両者の精度加重平均)の3通りを1本の数直線上に並べて比較するShrinkageLab(L0)を中心可視化とした。τ²(グループ間分散)スライダーとサンプルサイズ昇順のStepPlayerを同一ストアに統合し、τ²を動かすと全グループの推定値が即座に追従し、StepPlayerで1クラスずつ「サンプルサイズが小さいほど強く縮小される」ことを確認できる設計にした。L1では事後平均が«精度加重平均»になることを平方完成(高校数学の2次関数の最小値)だけで導出し、同じ式がベイズ線形回帰の傾きの事後平均(BayesianRegressionLab、n=8の小標本にOLSとベイズ事後平均を重ねる)にもそのまま現れ、Ridge回帰のMAP推定と数学的に同一であることを明示した。L2は2成分混合正規分布の事後所属確率(responsibility)を求めるLatentMixtureLab(潜在変数モデルの入門)を扱った。
- **検証結果**: Vitest **1448 passed**（+43: hierarchical-bayes計算層38・frames 5、registryリンク切れゼロ、既存152ファイル全て回帰なし）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ）。build成功(416ページ、新規`/topics/hierarchical-bayes`・5件の新規`/terms/*`すべて生成)。`pnpm start`本番ビルド + Playwright実機確認: ShrinkageLabのτ²スライダーを36→1に動かすとA組(n=5)のθ̂が64.7→63.2(全体平均μ≈63.1に接近)・縮小の重みB_jが0.36→0.95に変化、τ²=400にするとθ̂=65.5・B_j=0.05でプールなしの推定値(65.5)にほぼ一致することをDOM値で確認(数式term-thetaHat/term-Bjとグラフのcx座標が同期)。StepPlayerの「1コマ後へ」でA組→B組へフレームが進みハイライト行が移動、▶再生ボタンで自動再生が実際にF組まで進むことを確認(教訓#80の再発なし)。BayesianRegressionLabのτ_β²スライダーを0.5→500に動かすとβ_Bayesが0.47→4.90(OLSの4.94にほぼ一致)へ変化することを確認。LatentMixtureLabのxスライダーをx=40(成分0の平均)にするとP(z=0|x)=100%、x=58(中間付近)でP(z=0|x)=30%/P(z=1|x)=70%に変化することを確認。モバイル幅(390px)で全Lab視覚的に崩れないことをスクリーンショットで確認(`document.documentElement.scrollWidth`差9pxで既存トピックより良好)。コンソールエラー・警告0件。太字崩れ: CommonMarkフランキング規則のスキャナで新規6ファイル+編集2ファイルをスキャンし違反0件を確認。
- **設計判断 / つまずき**: **precisionWeightedMeanを階層ベイズとベイズ線形回帰で共有する設計** — 階層ベイズの事後平均θ̂_j=(w1ȳ_j+w2μ)/(w1+w2)とベイズ線形回帰の傾き事後平均β̂=(w1β̂_OLS+w2·0)/(w1+w2)は、どちらも「正規尤度×正規事前分布」という同じ共役構造から導かれるため、汎用関数precisionWeightedMean(dataMean, dataPrecision, priorMean, priorPrecision)を1つ作り両方の計算層から呼ぶ設計にした。MDX本文でもQ4のクイズでこの数学的必然性を問う設問を用意し、コード上の再利用が単なる実装上の都合ではなく数学的に正しい抽象化であることを明示した。**説明変数の中心化でベイズ線形回帰を閉形式にする** — 単回帰に多変量正規事前分布を素朴に置くと2x2行列の逆行列計算が必要になるが、説明変数を中心化(x-x̄)すると切片と傾きの最小二乗推定が直交し、傾きだけを独立にスカラーのprecisionWeightedMeanで扱えることに気づき、行列演算ライブラリなしで閉形式の解析解を実装できた——教育的にも「なぜ中心化するか」をMDX L1で明示した。**潜在変数モデルの学習者向け«答え合わせ»表示** — LatentMixtureLabの下段の点は実務では観測できない真の所属成分(trueComponent)を色分け表示しているが、これは学習者の理解を助けるための特別な表示であり実務では使えないことをSVG内のラベルテキストで明記し、誤解を避けた。**τ²=0の境界値でNaNを避けるガード** — shrinkageWeight関数でtau2<=0を「重み1(完全プールの極限)」として直接returnするガードを入れ、1/tau2=Infinityを経由した0/0=NaNを回避した(テストで境界値τ²=0・τ²=Infinityの両方を直接検証)。**bayesian-basics/mcmc-methodsのK-3前方参照を実装後にリンク化** — 両トピックのL3/L6 planned枠にあった「K-3「階層ベイズモデル」で扱う到達途上の領域」というテキストのみの前方参照を、本トピック実装後に`[階層ベイズモデル](/topics/hierarchical-bayes)`という実リンクに更新し、registry/buildで壊れていないことを確認した——今後も前提関係にある後続トピックが実装されたら同様に更新するのが望ましい。**K-3完了**、進捗 61/82。次は K-4 ベイズ応用（A/Bテスト・IRT、前提: bayesian-basics）へ。

### コンテンツ拡充 #88 — [K-4] ベイズ応用（A/Bテスト・IRT）
- [x] worktree内で作業ブランチ feat/88-bayesian-applications を作成
- [x] 出典精読: Bookmemo『Pythonでスラスラわかるベイズ推論「超」入門』第6章「ベイズ推論の業務活用事例」（A/Bテストの効果検証: クリック率をベータ分布でモデル化し継続的評価・確率的判断「A案がB案より優れている確率は85%」・効果の定量化・リスク評価、IRT: 受験者能力θ・問題困難度b・識別力a・当て推量cの2PL/3PLモデル）。既存トピック`content/topics/ab-testing.mdx`（頻度論の2標本比率検定・検出力分析）と`content/topics/bayesian-basics.mdx`（ベータ-二項共役）を精読し、重複回避のためベイズA/Bテストは「2群比較への応用」に絞り、共役更新自体はK-1の関数を再利用する設計にした
- [x] 計算層 `lib/stats/bayesian-applications.ts`（K-1の`updateBetaWithCounts`/`betaMean`/`betaPdfCurve`を再利用しベイズA/Bテストへ応用、Beta(a,b)(a,bが正整数)を**一様乱数の順序統計量**として超越関数を使わず厳密にサンプリングする`sampleBetaInteger`(二項分布とベータ分布を結ぶ古典的恒等式)、モンテカルロでP(θ_B>θ_A)・両方向の期待損失を同時推定する`runAbMonteCarlo`/`estimateAbComparison`、20件の固定観測系列を1件ずつ反映する`sequentialAbUpdates`、IRT2PLモデルの正答確率`irtProbability`(`lib/stats/logistic.ts`のsigmoidを再利用)・項目特性曲線`itemCharacteristicCurve`・Fisher情報量`itemInformation`・回答パターンから対数尤度曲線を求める`likelihoodCurve`/`mleTheta`(格子探索によるMLE)) + Vitest 34
- [x] 状態層 `lib/store/bayesian-applications.ts`（createTopicStore×1: controls={successesA/trialsA/successesB/trialsB, iccTheta, itemResponses}、ステッパーはページ全体でAbObservationStepperの1つだけなのでメインストアのframeを共用、tasks/lessons.md #76の判断目安通り。A/B観測ステッパーの全ステップは`AB_SEQUENTIAL_STEPS`としてcontrolsに依存しないモジュール定数に事前計算）
- [x] 描画層 `components/topics/bayesian-applications/`: AbBayesLab(L0, A/B各案の訪問者数・コンバージョン数スライダー+事後ベータ曲線の重ね描き+モンテカルロP(B>A)・期待損失+頻度論p値との対比パネル)/ AbObservationStepper(L1, 20件の観測系列を1件ずつ反映するStepPlayer)/ IccLab(L2, 能力θスライダー+識別力が異なる3項目のICC比較)/ IrtLikelihoodLab(L2, 5項目の正解/不正解トグル+尤度曲線+MLEマーカー)/ BayesianApplicationsQuiz(5問) + frames.ts(AbObservationStepper用) + Vitest 5
- [x] 用語ノード4件新規作成(bayesian-ab-testing・expected-loss・item-response-theory・item-characteristic-curve、相互seeAlsoで接続し既存のconjugate-prior/posterior-distribution/monte-carlo-method/logistic-regression/order-statistic/law-of-large-numbers/maximum-likelihood/log-odds/expected-valueへリンク)
- [x] content/topics/bayesian-applications.mdx(ReaderGuide + L0-2フル + L3-6 planned、モンテカルロでP(B>A)を推定する仕組みを大数の法則(高校数学の「割合」の拡張)から導出し、さらに発展として一様乱数の順序統計量でBeta(a,b)を厳密にサンプリングできる古典的恒等式(二項分布の裾の和を微分するとベータ密度に一致)を導出、IRTは対数尤度とFisher情報量a²P(θ)(1-P(θ))をシグモイドの導関数から導出。bayesian-basics(K-1)への前提リンク・ab-testing/qualitative-regressionへの関連リンク、ab-testing.mdx L4・hierarchical-bayes.mdx L6のK-4前方参照を実装済みの本トピックへの実リンクに更新)
- [x] 検証(vitest 1489 / tsc / lint / build / Playwright実機)
- [x] backlog ✅done、todo レビュー追記

#### レビュー: ベイズ応用（A/Bテスト・IRT）トピック（2026-07-25）
- **変更概要**: K群最終4本目・前提 bayesian-basics（間接的にab-testing・qualitative-regression）。ベイズA/Bテストは「A案・B案それぞれのコンバージョン率をベータ事後分布として推定し、モンテカルロでP(θ_B>θ_A)・期待損失を求める」という中核をAbBayesLab(L0, スライダー操作)とAbObservationStepper(L1, 20件の観測系列をコマ送り)の2つで見せ、同じデータに対する頻度論の2標本比率検定(p値)を並べて表示することで「有意/非有意の二分」と「確率・損失の大きさで判断する」という2つの意思決定パラダイムの対比を明確にした。IRTは2パラメータロジスティックモデル(能力θ・識別力a・困難度bからロジスティック回帰と同じシグモイド関数で正答確率を求める)を、識別力の異なる3項目のICCを比較するIccLab(L2)と、5項目の正誤パターンをトグルすると尤度曲線・最尤推定値θ_MLEがリアルタイムに更新されるIrtLikelihoodLab(L2)の2つで可視化した。数学的なハイライトは、Beta(a,b)(a,bが正整数)が「a+b-1個の一様乱数の順序統計量のa番目」と厳密に一致するという古典的恒等式を使い、`Math.log`等の超越関数を一切使わない比較・ソートだけの決定的サンプリング(`sampleBetaInteger`)を実装した点——SSR/CSRのハイドレーション不一致を根本から回避しつつ(tasks/lessons.md «整数演算だけの決定的LCG» の教訓をベータ分布サンプリングへ拡張)、MDX L1でその導出も高校数学の二項分布の裾の和から示した。
- **検証結果**: Vitest **1489 passed**（+41: bayesian-applications計算層34・frames 5、他既存テストは`.katex-html`スコープの修正含め回帰なし、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ）。build成功(421ページ、新規`/topics/bayesian-applications`・4件の新規`/terms/*`すべて生成)。`pnpm dev`(port 3211) + Playwright実機確認: AbBayesLabのB案コンバージョン数スライダーを14→30に動かすとP(θ_B>θ_A)が92.8%→100.0%、期待損失(Aを選ぶ)が0.1478→0.5272に変化し数式term-aB/term-bB等が連動して更新されることをDOM値(`.katex-html`スコープで読み取り)で確認。初期値(A:8/40, B:14/40)ではP(B>A)=92.8%なのに頻度論の両側p値=13.30%(有意でない)という、2つの枠組みが異なる結論を出す実例を実際に確認できた。AbObservationStepperの「1コマ後へ」を3回押すとstep-counterが1/21→4/21に進み、posteriorA=Beta(2,2)・posteriorB=Beta(2,1)・P(B>A)=70.0%へ正しく更新されることを確認(教訓#80の再発なし)。IccLabのθスライダーを1.5にすると識別力の異なる3項目の正答確率が74.1%/90.5%/98.5%(sigmoid(a·θ)の理論値と一致)に分かれて更新されることを確認。IrtLikelihoodLabで項目を正解/不正解にトグルすると尤度曲線とθ_MLEマーカーが更新され(回答数3・正答数2でθ_MLE=0.32)、既に選択中のボタンを再クリックすると未回答に戻る(トグルの冪等な設計)ことを確認。モバイル幅(390px)で`document.documentElement.scrollWidth`とinnerWidthの差**0px**(横スクロールなし、既存トピックより良好)を確認。コンソールエラー・警告0件。太字崩れ: CommonMarkフランキング規則のスキャナで新規5ファイルをスキャンし、**inline数式`$...$`に直接接する太字**(`**$n=a+b-1$`)と**閉じ括弧の直後に太字を閉じる**(`モデル(2PLモデル)**では`)の2パターンを新たに発見・修正した（前者はKaTeXの`$`がCommonMarkの記号/句読点カテゴリに分類されるため生じる、既存lessons.mdに未記載のパターン）。
- **設計判断 / つまずき**: **Beta(a,b)の順序統計量サンプリング** — モンテカルロでP(B>A)を推定するには乱数でBeta分布からサンプリングする必要があるが、通常のBox-Muller的な近似はガウス分布向けで、Beta分布に直接使える「超越関数なしの厳密なサンプラー」は一般には知られていない。事前分布Beta(1,1)(整数)+観測数(整数)という本トピックの設定では事後分布のパラメータが常に正整数になることに着目し、「Beta(a,b)(a,b∈ℤ⁺)はa+b-1個の一様乱数の順序統計量のa番目に厳密に一致する」という統計学の古典的事実を使うことで、比較・ソートだけで完結する決定的サンプラーを実装できた——近似ではなく数学的に厳密な手法を選べたことで、SSR/CSRの整合性とテスト(大量サンプルの標本平均・分散が理論値`betaMean`/`betaVariance`に一致することを直接検証)の両方を満たせた。**DOM id重複の実機発見と修正** — AbBayesLabとAbObservationStepperが同じ数式(θ_A~Beta(a,b)の形)を扱うため、実装時に安易に同じterm id(aA/bA/aB/bB/pHat)を両方に使ってしまった。Playwright実機確認で`document.querySelectorAll('#term-aA')`が2件返ることを発見し(tasks/lessons.md #78で既に警告されていたパターンだったが実装時に見落とした)、AbObservationStepper側を`stepAA`等の接頭辞付きに改名して名前空間を分離した——コードレビューだけでなく実機のDOM検証まで行って初めて気づけたバグで、今後複数コンポーネントで似た数式を扱う際はterm id設計を最初から意識する必要がある。**textContentでKaTeX値を読むと3重に見える罠** — Playwright実機確認でterm要素の値を`.textContent`で読むと、KaTeXが同じ内容を`.katex-mathml`(アクセシビリティ用)・`annotation`(生TeXソース)・`.katex-html`(視覚表示)の3箇所にレンダリングするため、例えば値"9"が"999"のように連結されて見える(実際の画面表示は正しく1つだけ)。`.katex-html`にスコープしたクエリで読むのが正しい検証方法——tasks/lessons.mdに新規教訓として追記した。**K群(K-1〜K-4)完了**、進捗 62/82。次は P-1 決定分析（前提: probability-basics）へ。

### コンテンツ拡充 #89 — [P-1] 決定分析（decision-analysis）
- [x] worktree内で作業ブランチ feat/89-decision-analysis を作成
- [x] 出典精読: Bookmemo『意思決定分析と予測の活用』第2部「決定分析の基本」第1・3・4章（利得行列の構造$D=\{A,\Theta,c\}$、確率なしの5決定基準(Maximax/Maximin/Hurwicz/Minimax regret/Laplace)、優越する決定、期待金額(ESV)による意思決定、決定木(決定ノード□・確率ノード○)と後ろ向き帰納法）。第2章のPandas例（工場の稼働台数0台/1台/2台×好況/不況の利得行列、単位:万円）をそのまま一次データとして採用（架空の数値を作らず出典に忠実）
- [x] 計算層 `lib/stats/decision-analysis.ts`（純関数: rowMax/rowMin/rowMean/colMax、maximaxCriterion/maximinCriterion/hurwiczCriterion/laplaceCriterion/regretMatrix/minimaxRegretCriterion/expectedValueCriterion、dominatedActionIndices(優越される行動)、決定木の型TreeNode(terminal/chance/decision判別共用体)・evaluateTreeNode(再帰的な後ろ向き帰納法)・collectPostOrder/backwardInductionSteps(ステッパー用)・buildDecisionTree(利得行列から木を機械的に構築)) + Vitest 26
- [x] 状態層 `lib/store/decision-analysis.ts`（useDecisionAnalysisStore: controls={payoffs(セル編集可能な2次元配列), criterion, hurwiczAlpha, probGood}で全5基準+リグレット行列+ESV+決定木をderiveで一括計算。RegretMatrixStepper・DecisionTreeStepperの2つがStepPlayerを持つため、tasks/lessons.md #76の判断目安どおりuseRegretStepperStore/useDecisionTreeStepperStoreを空controlsで個別に分離）
- [x] 描画層 `components/topics/decision-analysis/`: PayoffMatrixLab(L0、利得行列のセル編集+5基準ボタン+Hurwiczのαスライダー+選択行動のハイライト+数式の該当項を強連動、優越される行動にバッジ表示)/ RegretMatrixStepper(L1、状態(列)ごとに「最大利得を探す→リグレットを引き算」を1コマずつ見せ最後にMinimax regretの選択を示す)/ ExpectedValueLab(L2、好況確率pスライダー+ESV棒グラフ+数式強連動)/ DecisionTreeStepper(L2、SVGで決定木を描画し後ろ向き帰納法を1コマずつ進めるステッパー、選ばれた枝を緑色でハイライト)/ DecisionAnalysisQuiz(5問) + frames.ts(buildRegretFrames/buildTreeFrames) + Vitest 9
- [x] 用語ノード7件新規作成(payoff-matrix・maximax-criterion・maximin-criterion・hurwicz-criterion・minimax-regret・laplace-criterion・decision-tree-analysis)。既存の`decision-tree`(機械学習の分類木CART)と英語名が衝突するため、意思決定分析側は`decision-tree-analysis`という別slugで新設し、両方の用語ページに「同じ名前だが別概念」という相互注記を追加(tasks/lessons.md #85の«離散事象の確率と連続パラメータの分布は別ノードにする»と同型の判断)。expected-valueは既存を再利用(重複作成せず)
- [x] content/topics/decision-analysis.mdx(ReaderGuide + L0-2フル + L3-6 planned(P-2情報理論/P-3情報の価値/P-4効用理論/P-5確率予測の評価/P-6逐次決定を到達途上として言及、未実装トピックへは実リンクを張らずプレーンテキストで言及)。前提probability-basicsへリンク。L1で5基準の定義式をそれぞれ高校数学(max/min、加重平均=内分点、引き算、相加平均)に橋渡しして導出、L2でESVの定義を期待値の定義から導出し決定木の後ろ向き帰納法(確率ノード=期待値・決定ノード=最大値)を導出)
- [x] 検証(vitest 1524 / tsc / lint / build / Playwright実機)
- [x] backlog ✅done、todo レビュー追記
- [x] lessons.md追記(setValueで動的更新する項の値をPlaywrightで読むとき、コンテナ起点の`.katex-html`では入れ子で3重連結が再発する——`#term-<key> .katex-html`まで絞る必要がある、出典#89)

#### レビュー: 決定分析トピック（2026-07-25）
- **変更概要**: P群初回・前提 probability-basics。出典第2章のPandas例(工場の稼働台数0台/1台/2台×好況/不況の利得行列)を一次データとしてそのまま採用し、Level0のPayoffMatrixLabを中心可視化とした——利得行列のセルを直接編集でき、Maximax/Maximin/Hurwicz/Minimax regret/Laplaceの5決定基準をボタンで切り替えると選ばれる行動の行がハイライトされ、KaTeX数式中の該当項(選ばれたスコアやHurwiczのα)がリアルタイムに更新される「操作→図→数式」の強連動を1つのラボに統合した。Level1はリグレット行列の構築過程(状態(列)ごとに最大利得を探し引き算でリグレットを求める)をRegretMatrixStepperでコマ送りにし、Level2は期待値による意思決定(ESV)をExpectedValueLabの確率スライダーで、複数段階の意思決定を解く決定木をDecisionTreeStepperの後ろ向き帰納法コマ送りで可視化した——決定木の根の値は同じ利得行列・確率でのESVの最大値と数学的に厳密に一致することをテスト(`decision木の期待値はexpectedValueCriterionのスコアと一致する`)で直接検証し、MDX本文でも「表で計算するか木で計算するかの違いでしかない」と明示した。
- **検証結果**: Vitest **1524 passed**（+35: decision-analysis計算層26・frames 9、既存156ファイル全て回帰なし、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ）。build成功(429ページ、新規`/topics/decision-analysis`・7件の新規`/terms/*`すべて生成)。`pnpm dev`(port 3411) + Playwright実機確認: PayoffMatrixLabでMaximax(既定)→2台稼働が選択されスコア700が数式に反映、Maximinボタンで1台稼働・スコア300に切り替わることを`#term-score .katex-html`スコープで確認。Hurwiczボタン+αスライダーを0.7にすると2台稼働・スコア400(=1000×0.7−300の手計算と一致)に切り替わることを確認。利得行列のセル(2台稼働・好況)を700→1000に編集すると同じHurwicz(α=0.7)でスコアが610に再計算されることを確認(セル編集→全基準の再計算が連動)。RegretMatrixStepperをStepPlayerで5コマ進めると好況列(800/400/0)→不況列(400/0/600)の順にリグレットが1つずつ現れ、最終フレームで最大リグレット(800/400/600)のうち最小の1台稼働が選択されることを確認。ExpectedValueLabの確率pスライダーを0.8にするとESV(2台稼働)=500に更新され、DecisionTreeStepperをコマ送りすると確率ノードの値(-100/300/500)が1つずつ確定し最終フレームで決定ノードが500(2台稼働の枝)を選び該当エッジが緑色でハイライトされることを確認(利得行列Lab・確率スライダーとDecisionTreeStepperが同じストアを共有し連動することも確認)。演習クイズの正解ボタンで「正解!」フィードバックが表示されることを確認。モバイル幅(390px)で`document.documentElement.scrollWidth`とinnerWidthの差**0px**(横スクロールなし)を確認。コンソールエラー・警告0件。太字崩れ: CommonMarkフランキング規則のスキャナで新規9ファイルをスキャンし、**閉じ`**`の直後に仮名が続くパターン**(`**リグレット(後悔)**と呼ぶ`等5箇所)を発見・修正した(#73の既知パターンの再発、括弧を全角に変えるのではなく太字の範囲を語そのものだけに縮めて閉じ`**`の直後を句読点にする対策を徹底)。
- **設計判断 / つまずき**: **Bookmemo一次情報の忠実な再利用が教育的にも有効だった** — issue指示の例(傘/天候)ではなく出典第2章のPandas例(稼働台数×景気)をそのまま使ったところ、Hurwiczの交点(α=0.6)とESVの交点(p=0.6)が偶然一致するという教育的に興味深い構造(2台稼働の最良/最悪がちょうど好況/不況に対応するため)を発見でき、「主観的な楽観度αと客観的な確率pが同じ数式構造を持つ」ことを図らずも示せた。**`decision-tree`用語のslug衝突** — 新規に追加する決定木の用語が、既存の機械学習トピック(decision-trees-ensembles)が使う`decision-tree`(CART)とタイトル・概念ともに衝突することに気づき、tasks/lessons.md #85と同型の判断(対象が違うものは別ノードにし、重複するaliasは持たせず相互に注記する)で`decision-tree-analysis`という別slugを新設した。**setValueが作る入れ子`.katex-html`によるPlaywright検証の罠** — コンテナ起点の`.katex-html`スコープ(#88の対策)で値を読んでも、その項自体が`setValue`で動的更新される対象だと入れ子のkatex-html/mathml/annotationトリオが再度3重連結を起こすことを発見し、`#term-<key> .katex-html`まで絞る対策をlessons.mdに新規教訓として追記した。**P群開始**、進捗 63/82。次は P-2 情報理論（前提: decision-analysis, probability-basics）へ。

### コンテンツ拡充 #90 — [P-2] 情報理論（information-theory）
- [x] 作業ブランチ feat/90-information-theory を作成
- [x] 重複回避確認: 既存`content/terms/entropy-impurity.mdx`（決定木の不純度指標としてのエントロピー）・`information-gain.mdx`（決定木の情報利得）・`content/topics/decision-trees-ensembles.mdx`のエントロピー記述を精読。既存`kullback-leibler-divergence.mdx`（KLダイバージェンス用語）と`content/terms/bit.mdx`・`content/topics/digital-information-basics.mdx`（bit＝情報量の単位、KLダイバージェンスへ前方リンク済み）を発見し、KL用語は新規作成せず既存を再利用（重複回避）
- [x] 計算層 `lib/stats/information-theory.ts`（純関数: selfInformation(-log2 p)・entropy・binaryEntropy・marginalX/Y・jointEntropy・conditionalEntropy・productDistribution・klDivergence(非対称・q=0でInfinity)・mutualInformation・mutualInformationViaKL・mutualInformationViaEntropies(3つの等価な定義が数値一致することをテストで確認)・normalize/normalizeJoint(0除算フォールバック)) + Vitest 29
- [x] 状態層 `lib/store/information-theory.ts`（useInformationTheoryStore: controls={coinP, jointCounts(2×2の同時分布), klPWeights/klQWeights(4カテゴリ×2分布)}。SelfInformationStepperがこのトピック唯一のStepPlayerなのでtasks/lessons.md #76の判断目安どおりメインストアのframeを共用）
- [x] 描画層 `components/topics/information-theory/`: BinaryEntropyLab(L0/L1、確率pスライダー+H(p)曲線(p=0.5で最大)+数式強連動)/ SelfInformationStepper(L1、確実な事象→珍しい事象まで5例をコマ送り)/ MutualInformationLab(L2、天気×傘の同時分布4セル編集+周辺分布表+I(X;Y)を直接式・KL経由の2通りで同時表示)/ KlDivergenceLab(L2、天気予報4カテゴリの2分布を棒グラフで編集しD_KL(P‖Q)とD_KL(Q‖P)の非対称性を並べて表示)/ InformationTheoryQuiz(5問) + frames.ts(buildSelfInformationFrames) + Vitest 5
- [x] 用語ノード3件新規作成(shannon-information・entropy・mutual-information)。既存`entropy-impurity`は「一般的な情報理論のエントロピーを決定木の不純度に応用したもの」と明記した上でseeAlsoに`entropy`を追加(本文書き換えなし、リンク追加のみ)。既存`kullback-leibler-divergence`は再利用(重複作成せず)
- [x] content/topics/information-theory.mdx(ReaderGuide + L0-2フル + L3-6 planned(P-3情報の価値・特徴選択への応用/情報理論的因果尺度/微分エントロピー・情報幾何/機械学習との接続を到達途上として言及)。前提probability-basicsへリンク、decision-analysis(P-3の土台)・decision-trees-ensembles(応用例)へも相互リンク。L1で自己情報量が-log2 pになることを「独立なら加法的」という要請とlog則から導出、エントロピーがp=0.5で最大になることを微分で導出。L2でKLダイバージェンスの非負性をGibbsの不等式(ln x≤x-1の接線不等式、高校数学の「上に凸なグラフは接線より下」に橋渡し)で導出し、相互情報量とKLの関係I(X;Y)=D_KL(P(X,Y)‖P(X)P(Y))を導出)
- [x] 検証(vitest 1558 / tsc / lint / build / Playwright実機)
- [x] backlog ✅done、todo レビュー追記
- [x] lessons.md追記(MDX太字の`**<Term id="...">`が地の文の仮名の直後に直接続くと崩れる新パターン、出典#90)

#### レビュー: 情報理論トピック（2026-07-25）
- **変更概要**: P群2件目・前提probability-basics。「珍しい事象ほど情報量が大きい」という直感を$I(x)=-\log_2 p(x)$という数式に落とし込む導出から出発し、エントロピー・相互情報量・KLダイバージェンスまでを一気通貫に扱った。Level0/1の中核はBinaryEntropyLab——2値事象の確率pをスライダーで動かすと、H(p) vs pの曲線(p=0.5で山)と数式中の値がKaTeXの`\htmlId`項ハイライトで実時間連動する。Level1はSelfInformationStepperで「必ず起きる事象(p=1, 0 bit)」から「100人に1人の珍しい事象(p=0.01, 約6.64 bit)」まで5例をコマ送りにし、自己情報量の直感を段階的に積み上げた。Level2は相互情報量(天気×傘の2×2同時分布を編集するMutualInformationLab)とKLダイバージェンス(天気予報4カテゴリの2分布を棒グラフで編集するKlDivergenceLab)の2ラボを配置し、前者では「相互情報量を直接の定義式とKLダイバージェンス経由の2通りで同時に計算し数値が一致することを表示」、後者では「D_KL(P‖Q)とD_KL(Q‖P)を並べて表示し非対称性を数値で直接示す」ことで、$I(X;Y)=D_{KL}(P(X,Y)\Vert P(X)P(Y))$という関係を可視化レベルでも裏付けた。
- **検証結果**: Vitest **1558 passed**（+34: 計算層29(自己情報量の加法性・エントロピーの最大性・相互情報量の3つの等価な定義の数値一致・KLの非対称性と非負性・q=0での発散を含む)・frames 5、既存157ファイル全て回帰なし、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ）。build成功(433ページ、新規`/topics/information-theory`・3件の新規`/terms/*`すべて生成)。`pnpm dev`(port 3457) + Playwright実機確認: BinaryEntropyLabのpスライダーを0.2にするとH(p)=0.7219・自己情報量(表)=2.322 bitに再計算され、SVG上の現在点が曲線上を正しい座標(cx=85.2, cy=45.04)に移動することを確認。SelfInformationStepperの▶再生ボタンでStepPlayerが自動的に5/5まで進み末尾で自動停止することを確認(tasks/lessons.md #80の再発なし)。MutualInformationLabで4セルを均等(独立)にするとI(X;Y)がちょうど0になることを`#term-mi .katex-html`スコープで確認(独立の理論値と一致)。KlDivergenceLabでQのカテゴリを0にするとD_KL(P‖Q)が統計表示側で「∞ bit」になることを確認(数式側の`formatNumber`は非有限値を"—"表示する既存仕様どおり)。演習クイズの正解ボタンで「正解!」フィードバックを確認。モバイル幅(390px)で`document.documentElement.scrollWidth`とinnerWidthの差**0px**(横スクロールなし)を確認。コンソールエラー・警告0件。
- **設計判断 / つまずき**: **既存用語の重複回避で2つの判断** — (1)`entropy-impurity`(決定木の不純度)とは別に一般的なシャノンエントロピーの新規term `entropy`を作成し、tasks/lessons.md #85と同型の判断(対象が違うものは別ノードにし本文に橋渡し文を追加)で相互リンクした。(2) 調査の過程で`content/terms/kullback-leibler-divergence.mdx`と`content/terms/bit.mdx`・`content/topics/digital-information-basics.mdx`が既に存在し、後者は本トピックへの前方リンク(「情報理論の入り口になる」)まで用意されていたことを発見——issueには「kl-divergence」という新規slugが例示されていたが、既存の`kullback-leibler-divergence`をそのまま`<Term id>`で参照し、重複作成を避けた。**MDX太字の新パターン発見** — Playwrightで`document.body.innerText.indexOf('**')`を機械的に検査したところ、`**<Term id="entropy">エントロピー</Term>**`のように地の文の仮名の直後に直接`**<Term`が続く1箇所で太字が崩れていることを発見(#73/#78/#88に続く第4のパターン、原因は`<`もCommonMarkのpunctuation判定に含まれるため)。既存コードベースには`**<Term`という書き方が数十件あったが、目視確認した範囲では行頭(直前が改行=空白扱い)のケースは問題なく、地の文途中のケースのみ崩れることを特定し、`tasks/lessons.md`に新教訓として記録した(既存ファイルの一括修正は本Issueのスコープ外と判断)。**P群2件目**、進捗 64/82。次は P-3 情報の価値（前提: decision-analysis, bayesian-basics）へ。

### コンテンツ拡充 #91 — [P-3] 情報の価値（value-of-information）
- [x] worktree内で作業ブランチ feat/91-value-of-information を作成
- [x] 出典精読: Bookmemo『意思決定分析と予測の活用』第2部「決定分析の基本」第6章「情報の価値」（VOI_post/VOI_pre/EVPIの定義式、期待リグレット=EVPI）・第3部「決定分析の活用」第4章「標準型分析」（Ⅰ型情報/Ⅱ型情報、判断確率とベイズ決定）。前提2トピック`content/topics/decision-analysis.mdx`（利得行列・ESV・リグレット行列を再利用）と`content/topics/bayesian-basics.mdx`（ベータ-二項共役のベイズ更新パターンを離散状態へ転用）を精読し、重複回避のため利得行列は決定分析(P-1)のPRIMARY_PAYOFF_MATRIXをそのまま再利用（本トピックでは編集不可、確率・的中率の操作に絞る設計）
- [x] 計算層 `lib/stats/value-of-information.ts`（decision-analysisの`expectedValueCriterion`/`regretMatrix`を再利用し、完全情報の期待利得`perfectInformationEsv`・状態ごとの最善行動`perfectInfoBestActionPerState`・EVPI`evpi`・期待リグレット`expectedRegretOfAction`(EVPIと一致することをテストで直接検証)、離散状態への一般ベイズ更新`bayesPosteriorFromSignal`・対称的中率の二値予報`symmetricForecast`・事前/事後分析一式`priorPosteriorAnalysis`(VOI_post・VOI_pre/EVSI)を実装) + Vitest 21
- [x] 状態層 `lib/store/value-of-information.ts`（useValueOfInformationStore: controls={probGood, forecastAccuracy}のみ(利得行列は固定)。ステッパーはEvpiStepperの1つだけなのでtasks/lessons.md #76の判断目安どおりメインストアのframeを共用）
- [x] 描画層 `components/topics/value-of-information/`: EvpiStepper(L0、「①事前情報のみの選択→②状態ごとに完全情報ならの選択(★マーカー)→③EVPI」を1コマずつ見せる)/ EvpiLab(L1、確率pスライダー+ESV棒グラフ+完全情報バー+EVPI(p)曲線+数式強連動)/ PriorPosteriorLab(L2、的中率qスライダー+シグナルごとの事後確率/条件付きESV/VOI_post+VOI_pre対EVPIのゲージ)/ ValueOfInformationQuiz(5問) + frames.ts(buildEvpiFrames) + Vitest 6
- [x] 用語ノード2件新規作成(evpi・prior-posterior-analysis、既存のexpected-value/payoff-matrix/minimax-regret/posterior-distributionへ相互リンク)
- [x] content/topics/value-of-information.mdx(ReaderGuide + L0-2フル + L3-6 planned。EVPIの定義式をmaxの分配則(高校数学のΣとmaxの順序交換)から導出、EVPI=期待リグレットの恒等式をリグレットの定義式から導出、離散状態へのベイズ更新をK-1の「事後∝尤度×事前分布」の直接の拡張として導出。decision-analysis/bayesian-basicsへ前提リンク。両トピック・information-theory.mddxのP-3前方参照(planned枠のプレーンテキスト)を本トピックへの実リンクに更新)
- [x] 検証(vitest 1585 / tsc / lint / build / Playwright実機)
- [x] backlog ✅done、todo レビュー追記

#### レビュー: 情報の価値トピック（2026-07-25）
- **変更概要**: P群3件目・前提decision-analysis(P-1)・bayesian-basics(K-1)の両方（P群で唯一の複数前提トピック）。決定分析(P-1)の工場稼働台数×景気の利得行列をそのまま再利用し、「事前情報のみでの意思決定」と「完全情報(状態を確実に知れる)での意思決定」の差=EVPIという核心を、EvpiStepper(L0)で「①事前情報のみで1台稼働を選ぶ→②好況なら2台稼働・不況なら1台稼働が最善(★マーカーで食い違いを可視化)→③その差=EVPI(200)」という3段階のコマ送りで見せた。L1のEvpiLabは確率pスライダーでEVPI(p)の曲線をリアルタイムに動かし、p=0/1(状態確定)でEVPIが0に近づく境界挙動を確認できる。L2のPriorPosteriorLabは、決定分析にはなかった「不完全な予報」を導入し、bayesian-basics(K-1)と同じベイズの定理を離散的な自然の状態に適用する事前/事後分析を実装——的中率qスライダーを0.5(無情報)から1(完全情報)まで動かすと、事前分析VOI_pre(EVSI)が0からEVPIまで単調に近づく様子をゲージで可視化し、「EVPIはあらゆる情報の価値の上限である」という決定分析とベイズ統計を橋渡しする核心を数値で裏付けた。
- **検証結果**: Vitest **1585 passed**（+27: value-of-information計算層21・frames 6、既存160ファイル全て回帰なし、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ）。build成功(436ページ、新規`/topics/value-of-information`・2件の新規`/terms/*`(evpi・prior-posterior-analysis)すべて生成)。`pnpm dev` + Playwright実機確認: EvpiStepperを4コマ進めると①「1台稼働」選択(300)→②好況列に★(2台稼働700)・不況列に★(1台稼働300)→③最終フレームでterm-perfect=500・term-prior=300・term-gap=200が表示され手計算(500-300=200)と一致することを`#evpi-stepper #term-gap .katex-html`スコープで確認。EvpiLabのpスライダーを0.7に動かすとterm-perfect/prior/gapが580/400/180に再計算され、EvpiStepperのキャプション文言も同じストアを共有して「p(好況)=70%」に連動して変わることを確認。PriorPosteriorLabで(p=0.7, q=0.8)のとき予報「好況」のVOI_post=203・予報「不況」のVOI_post=-100・VOI_pre=88(EVPI=180に対する比49%)が手計算と一致することを確認、qを1にするとVOI_pre=180(EVPIと完全一致、比100%)、q=0.5にするとVOI_pre=0(比0%)になる境界挙動を確認。演習クイズの正解ボタンで「正解!」フィードバックを確認。モバイル幅(390px)で全Lab・ステッパーが視覚的に崩れないことをスクリーンショットで確認(`document.documentElement.scrollWidth`差58px、tasks/lessons.md #84の既知のKaTeX起因の軽微な問題と同種でスコープ外と判断)。コンソールエラー・KaTeX描画エラー0件。太字崩れ: CommonMarkフランキング規則のスキャナで新規2ファイル+編集3ファイル(decision-analysis/information-theory/bayesian-basics)をスキャンし違反0件を確認。
- **設計判断 / つまずき**: **利得行列を編集不可にして操作対象を絞った** — decision-analysis(P-1)は利得行列そのものの編集がテーマだったが、P-3では「同じ利得行列に対して確率・情報の質を動かすとEVPI/VOIがどう変わるか」が主題なので、PRIMARY_PAYOFF_MATRIXを固定値としてそのまま再利用し、controlsをprobGood・forecastAccuracyの2つのスライダーだけに絞った——前提トピックとの重複を避けつつ、本トピック固有の主題(情報の価値)にラボの操作を集中させる設計判断。**EVPI=期待リグレットの恒等式を«2通りの読み方»として計算層のテストで直接検証** — Bookmemo第2部第6章に「期待リグレット=EVPI」と明記されていたため、`evpi()`と`expectedRegretOfAction()`という独立に実装した2つの関数が同じ入力で同じ値を返すことをテストで直接アサートし(p=0.5で200・p=0.7で180の両方)、MDX L1でも代数的にこの恒等式を導出した——「別の概念に見える2つの量が同じ数式の2通りの読み方だった」という決定分析とP-3をつなぐ最大の学びを、テスト・数式導出・可視化(EvpiStepperの★マーカー)の3つで裏付けられた。**対称な二値予報という最小のモデルでEVSIの直感を示した** — 現実の予報はもっと複雑な尤度構造を持ちうるが、「的中率q一つだけで動く対称な二値予報」に絞ることで、q=0.5(無情報)→VOI_pre=0とq=1(完全情報)→VOI_pre=EVPIという2つの境界がスライダー1本で確認でき、「情報の質と情報の価値の関係」という抽象的な主張を最小の操作で体感できる設計にした（P-6逐次決定やP-5確率予測の評価で、より一般的な予報モデルを扱う際の土台になる）。**P群3件目**、進捗 65/82。次は P-4 効用理論（前提: decision-analysis）へ。

### コンテンツ拡充 #92 — [P-4] 効用理論（utility-theory）
- [x] worktree内で作業ブランチ feat/92-utility-theory を作成
- [x] 出典精読: Bookmemo『意思決定分析と予測の活用』第4部「効用理論入門」（サンクトペテルブルクのパラドックス、vNM公理・vNMの定理、リスク態度3分類、確実同値額(CE)、リスクプレミアム、期待金額と期待効用の限界）。前提トピック`content/topics/decision-analysis.mdx`（利得行列・ESVを再利用）を精読し、重複回避のため利得行列は決定分析(P-1)のPRIMARY_PAYOFF_MATRIXをそのまま再利用（本トピックでは編集不可、リスク回避度α・好況になる確率pの操作に絞る設計）
- [x] 計算層 `lib/stats/utility-theory.ts`（サンクトペテルブルクの部分和`stPetersburgPartialExpectedValue`(発散)・`stPetersburgPartialExpectedUtility`(log₂効用で2に収束)、CARA効用の正規化形`utilityCara`(α→0で直線u(x)=xに連続接続)、凹凸の数値判定`concavityProbe`、確実同値額(閉形式`certaintyEquivalentCara`+数値探索`certaintyEquivalentBisection`の2通り)、リスクプレミアム`riskPremium`、期待効用による意思決定`expectedUtilityCriterion`とESVとの比較`compareEvAndEuDecisions`(p=0.7・α=0.002でESVは2台稼働・EUは1台稼働を選ぶ食い違いを実証)を実装) + Vitest 29
- [x] 状態層 `lib/store/utility-theory.ts`（useUtilityTheoryStore: controls={alpha, probGood}のみ(利得行列は固定)。ステッパーがStPetersburgStepper・RiskAttitudeStepperの2つあるため、tasks/lessons.md #76の基準どおりメインストアのframeを共用せず、個別の空controlsストアに分離）
- [x] 描画層 `components/topics/utility-theory/`: StPetersburgStepper(L0、コインを1回ずつ投げ足し期待金額の部分和(発散)とlog₂期待効用の部分和(2に収束)を1コマずつ比較) / RiskAttitudeStepper(L1、リスク回避的→中立→受容的の3コマで効用曲線の凹凸とCE・リスクプレミアムの位置関係(弦と曲線)を比較) / UtilityDecisionLab(L2、α・pの2スライダー+ESV/EU棒グラフ+効用曲線(E[X]・CEマーカー)+食い違い検出コールアウト+数式強連動) / UtilityTheoryQuiz(6問) + frames.ts(buildStPetersburgFrames・buildRiskAttitudeFrames) + Vitest 13
- [x] 用語ノード6件新規作成(utility-function・von-neumann-morgenstern-theorem・risk-attitude・certainty-equivalent・risk-premium・st-petersburg-paradox、既存のexpected-value/payoff-matrix/convex-functionへ相互リンク)
- [x] content/topics/utility-theory.mdx(ReaderGuide + L0-2フル + L3-6 planned。サンクトペテルブルクの期待金額発散を「定数項だけの数列の和」、log₂効用の収束を高校数学の「等差×等比数列の和(ずらして引く)」で導出、リスクプレミアムの非負性をイェンセンの不等式の直感(弦が凹関数の下にある高校数学の«上に凸»の定義)から証明。decision-analysisへ前提リンク。decision-analysis L4・value-of-information L4のP-4前方参照(planned枠のプレーンテキスト)を本トピックへの実リンクに更新——既存の公開済みファイルなので`prettier --write`はかけず`Edit`でピンポイントに書き換え、MDXの空行=段落分割による意図しないレイアウト崩れを回避)
- [x] バグ発見・修正(スコープ内): (1) `next.config.mjs`に`remark-gfm`が入っておらず、MDXのパイプテーブル記法(`|..|`)が`<table>`に変換されずただの段落として描画される既存バグを発見(`content/terms/confusion-matrix.mdx`等の既存ファイルも影響を受けていた)。remark-gfmを追加して修正、`mdx-components.tsx`にtable/thead/th/tdのoverflow-x-auto付きデフォルトスタイルを追加。(2) モバイル幅で長い一行の$$displayMath$$がページ全体を横スクロールさせる問題(KaTeXは折り返さない)を発見、4箇所の数式を`\begin{aligned}`で複数行に分割して解消。(3) UtilityDecisionLabのバー幅計算(`Math.exp`を含むCARA効用由来)でSSR/CSRのtranscendental 1ULP差によるhydration mismatchを発見、`round2()`で丸めて解消(tasks/lessons.md既知パターンの新規再発)
- [x] 検証(vitest 1627 / tsc / lint / build / Playwright実機)
- [x] backlog ✅done、todo レビュー追記

#### レビュー: 効用理論トピック（2026-07-25）
- **変更概要**: P群4件目・前提decision-analysis(P-1)。決定分析(P-1)L2で触れた「ESV最大化はリスク態度を反映できない」という限界に答えるトピック。サンクトペテルブルクのパラドックス(期待金額が発散するくじに有限の金額しか払わない逆説)を出発点に、StPetersburgStepper(L0)でコインを1回ずつ投げ足しながら期待金額の部分和(常にNに一致し発散)とlog₂効用の期待効用の部分和(2に収束)を並べて見せた。RiskAttitudeStepper(L1)は決定分析と同じ2台稼働のくじ(好況700万円/不況−300万円)に対して、CARA効用u(x;α)=(1-e^{-αx})/αのαを回避的(0.01)→中立(0)→受容的(−0.01)と切り替え、効用曲線と弦(2点を結ぶ直線)の上下関係からCEが期待値のどちら側にズレるかを教科書通りの図で示した。UtilityDecisionLab(L2)は決定分析(P-1)のPRIMARY_PAYOFF_MATRIXをそのまま再利用し、α・pの2スライダーでESV(期待金額)最大化とEU(期待効用)最大化が実際に食い違う具体例(p=0.7・α=0.002でESVは「2台稼働」(400)・EUは「1台稼働」(225.6)を選ぶ)をリアルタイムに再現し、期待金額と期待効用で意思決定が異なりうることを操作で体感できるようにした。
- **検証結果**: Vitest **1627 passed**（+42: utility-theory計算層29(CARA効用のα→0連続性・凹凸判定・CE数値探索と閉形式の一致・ESVとEUの食い違いの具体例・サンクトペテルブルクの発散と収束を含む)・frames 13、既存162ファイル全て回帰なし、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ）。build成功(443ページ、新規`/topics/utility-theory`・6件の新規`/terms/*`すべて生成)。`pnpm dev` + Playwright実機確認: UtilityDecisionLabのαスライダーを0に動かすと「ESVとEUは同じ行動を選んでいる」に切り替わりterm-ce/term-rpが400/0(=E[X])に再計算されることを確認、α=0.004に戻すとCE=−9・RP=409(強いリスク回避)に再計算されることを確認。RiskAttitudeStepperで3コマ進めるとリスク回避的(凹、CE=-180<E[X]=400)→中立(直線、CE=E[X]=400)→受容的(凸、CE=664>E[X]=400)と効用曲線・弦の上下関係・CE位置が正しく切り替わることを確認(教科書通りのイェンセンの不等式の図)。StPetersburgStepperの▶再生ボタンでStepPlayerが実際に進み(教訓#80の再発なし)、n=5でterm-ev=5・term-eu=1.781(=Σn/2ⁿのn=5部分和)が手計算と一致することを`#term-ev .katex-html`スコープで確認。演習クイズの正解ボタンで「正解!」フィードバックを確認。モバイル幅(390px)で`document.documentElement.scrollWidth`とclientWidthの差**0px**(全<details>展開状態でも横スクロールなし)を確認——実装中に486px差の実横スクロール(見た目にも数式が画面外に切れる)を発見し、原因(後述)を特定・修正して0pxまで追い込んだ。コンソールエラー・KaTeX描画エラー0件。
- **設計判断 / つまずき**: **CARA効用を「α=0で直線に連続接続する正規化形」に変更** — Bookmemo第2章のPython例は生の`u(x)=1-e^{-αx}`だが、この式はα→0で恒等的に0へ潰れ「リスク中立(直線u(x)=x)」につながらない。vNM効用は正のアフィン変換で不変(カーディナルな尺度)という性質を使い、`u(x;α)=(1-e^{-αx})/α`という標準的なCARA正規化形(α→0の極限でxに一致)を採用——意思決定への影響はBookmemoの式と完全に同じまま、αのスライダー1本でリスク回避的↔中立↔受容的を連続的に動かせるようにした。**decision-analysisの利得行列をそのまま再利用してEV/EU食い違いの具体例を構築** — 新しいくじを作らず、前提トピックと同じ工場稼働の利得行列(2台稼働=好況700/不況−300というハイリスク・ハイリターンな行動)にCARA効用を適用するだけで「ESVは2台稼働・EUは1台稼働を選ぶ」という強い食い違いが自然に得られることをNode上の手計算で先に確認してから実装・テストに落とし込んだ(p=0.7, α=0.002を既定値にしたのは、ページを開いた瞬間から食い違いが見えるようにするため)。**モバイル横スクロールの実バグを2種類発見・修正** — (1) `next.config.mjs`に`remark-gfm`が無く、MDXの`|..|`テーブル記法が`<table>`にならずただの段落として描画される既存バグを発見(`content/terms/confusion-matrix.mdx`等の既存ファイルも影響を受けていた、本トピック固有ではない全体バグ)。remark-gfmを追加し、`mdx-components.tsx`にtable系のoverflow-x-auto付きデフォルトスタイルも追加(SPEC全体に波及する良い副産物)。(2) L0の`E[X]=½×2+¼×4+...`のような1行の長い$$display math$$がKaTeXの非折り返し仕様によりモバイル幅を突き破ることを発見、4箇所を`\begin{aligned}...\end{aligned}`で複数行に分割して解消——tasks/lessons.mdの既存教訓(#84 KaTeX mathml)とは別種の「実際に見た目が切れる」新パターンとして追記が必要と判断。**P群4件目**、進捗 66/82。次は P-5 確率予測の評価（前提: binary-classification-metrics）へ。

### コンテンツ拡充 #93 — [P-5] 確率予測の評価（probabilistic-forecasting）
- [x] worktree内で作業ブランチ feat/93-probabilistic-forecasting を作成
- [x] 出典精読: Bookmemo『意思決定分析と予測の活用』第5部「確率予測とその活用」第1章（一貫性/信頼度・ブライアスコア・ブライアスコアの分解(Murphy分解)・Sharpness・ROC曲線とAUC）、第3部「決定分析の活用」第2章「コスト／ロスモデルと予測の価値」。前提トピック`content/topics/binary-classification-metrics.mdx`（ROC/AUCが既出）を精読し、本トピックではROC/AUCを再実装せず前提リンク+要点整理のみに留める設計とした
- [x] 計算層 `lib/stats/probabilistic-forecasting.ts`（決定的LCGによる合成データセット`generateForecastBase`(層化trueProb+ベルヌーイoutcome)、確信度スケールによる予測の歪め`distortForecast`+`roundToGrid`によるnBins刻みの離散化、`brierScore`、ステッパー用`cumulativeBrierTerms`、`calibrationBins`、Murphy分解`murphyDecomposition`(信頼度エラー-Refinement+不確実性が離散化済み予測に対し厳密な恒等式になることを高校数学の平方完成で導出・実装)、`sharpness`(予測確率の分散)、コスト/ロスモデルの最適閾値`costLossOptimalThreshold`(p*=C/L)と予測の価値評価`evaluateCostLoss`(気候値の最良〜完全な予測を上下限とする価値スコアV)) + Vitest 36
- [x] 状態層 `lib/store/probabilistic-forecasting.ts`（useProbabilisticForecastingStore: controls={confidenceScale, costC, lossL}。ステッパーはBrierScoreStepper1つだけなのでtasks/lessons.md #76の判断の目安どおりメインストアのframeをそのまま共用、専用の空ストアは作らない）
- [x] 描画層 `components/topics/probabilistic-forecasting/`: BrierScoreStepper(L0、10件の予測を1件ずつ確認しながら二乗誤差が積み上がる様子をコマ送り) / CalibrationLab(L1〜L2、確信度スケールkスライダーで信頼度曲線(Reliability Diagram)・Sharpnessヒストグラム・ブライアスコアの3分解がリアルタイム連動) / CostLossLab(L2、C/Lスライダーで最適閾値p*と気候値/予測/完全な予測の期待コストの並び・価値スコアVが連動) / ProbabilisticForecastingQuiz(6問) + frames.ts(buildBrierScoreFrames) + Vitest 5
- [x] 用語ノード4件新規作成(brier-score・calibration・sharpness・cost-loss-model)。既存のroc-curve・auc・confusion-matrix・expected-value・payoff-matrix・cost-sensitive-learning・evpiへ相互リンク（重複作成せず再利用）
- [x] content/topics/probabilistic-forecasting.mdx(ReaderGuide + L0-2フル + L3-6 planned。ブライアスコアの0〜1範囲を高校数学の不等式で導出、Murphy分解を「y∈{0,1}のためy²=y」を使った平方完成の展開で高校数学レベルに橋渡しして導出、コスト/ロスモデルの最適閾値p*=C/Lを1次不等式の両辺を正の数で割る操作として導出。binary-classification-metrics/decision-analysis/value-of-informationへ前提リンク。decision-analysis L5・value-of-information L3のP-5前方参照(planned枠のプレーンテキスト)を本トピックへの実リンクに更新——既存の公開済みファイルなので`prettier --write`はかけず`Edit`でピンポイントに書き換え)
- [x] バグ発見・修正(スコープ内): Level1のMurphy分解導出にある3本の$$display math$$(うち1本は\underbrace3つを含む長い等式)がモバイル幅390pxで実際に横スクロールを起こす(#92で見つけたパターンと同種の新規発生)ことをPlaywrightの`document.documentElement.scrollWidth`実測で発見、3本とも`\begin{aligned}`で複数行に分割して解消(修正後はscrollWidth=clientWidth=390で横スクロール完全ゼロ)
- [x] 検証(vitest 1668 / tsc / lint / build / Playwright実機)
- [x] backlog ✅done、todo レビュー追記

#### レビュー: 確率予測の評価トピック（2026-07-25）
- **変更概要**: P群5件目・前提binary-classification-metrics(J-3、ROC/AUC)。「陽性らしさスコア」の順位づけ性能(ROC/AUC)ではなく、「確率値そのものの目盛りが合っているか」という新しい観点(キャリブレーション)に焦点を絞った。核心はブライアスコア BS=(1/n)Σ(p̂-y)²——BrierScoreStepper(L0)で10件の予測を1件ずつ確認しながら二乗誤差が積み上がる様子をコマ送りにし、「自信満々で当てると誤差はほぼ0、自信満々で外すと誤差が跳ね上がる」ことを体感できるようにした。CalibrationLab(L1〜L2)は200件の合成データ(層化trueProb+LCGベルヌーイoutcome)に確信度スケールkで過信/自信不足の歪みを加え、信頼度曲線(Reliability Diagram)・Sharpnessヒストグラム・ブライアスコアの3分解(信頼度エラー・Refinement・不確実性)が同時に連動する様子を見せた。CostLossLab(L2)は決定分析(P-1)・情報の価値(P-3)と同じ「期待コストの比較で行動を選ぶ」枠組みをコスト/ロスモデルに適用し、「気候値(予測なし)の最良〜完全な予測」を下限・上限とする予測の価値スコアVで、確率予測がどれだけ意思決定の価値を生むかを定量化した——CalibrationLabの確信度スケールとCostLossLabのC/Lが同じストアのpredicted配列を共有するため、「較正を崩すと価値スコアVが下がる」という2ラボ間の接続も操作で確認できる設計にした。
- **検証結果**: Vitest **1668 passed**（+41: probabilistic-forecasting計算層36(Murphy分解の恒等式を手計算した具体例で3項とも検算・コスト/ロスモデルの完全な予測/情報のない予測での境界値・LCGの決定性を含む)・frames 5、既存164ファイル全て回帰なし、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ）。build成功(448ページ、新規`/topics/probabilistic-forecasting`・4件の新規`/terms/*`(brier-score・calibration・sharpness・cost-loss-model)すべて生成)。`pnpm build`後の`pnpm start`+Playwright実機確認: BrierScoreStepperの「1コマ後へ」ボタンで2件目(predicted=0.2,outcome=0)に進み`#term-se .katex-html`=0.04・`#term-bs .katex-html`=0.025(累積0.05÷2)が手計算と一致することを確認。CalibrationLabの確信度スケールスライダーをk=2にすると`#term-rel .katex-html`=0.0308(信頼度エラーが非ゼロに悪化)・Sharpnessヒストグラムの両端ビンが伸びることを確認。CostLossLabの対策コストスライダーをC=5にすると`#term-r .katex-html`=0.5(閾値再計算)・価値スコアV=0.407とコールアウト文言がリアルタイムに連動することを確認。演習クイズの正解ボタンで「正解!」フィードバックを確認。モバイル幅(390px)で全<details>展開状態の`document.documentElement.scrollWidth`が**clientWidthと完全一致(390px、差0px)**であることを確認——実装中にMurphy分解の導出にある長い$$display math$$(特に3本のunderbraceを含む等式)が486px相当はみ出す実横スクロールを発見し、3本とも`\begin{aligned}`で複数行に分割して0pxまで解消した。コンソールエラー・KaTeX描画エラー0件、太字崩れ(`**`残存)0件。
- **設計判断 / つまずき**: **Murphy分解を「離散化済み予測」に対する厳密な恒等式として実装** — Bookmemoの記法($f_k$=k番目のビンに属する予測の頻度)が既にビン単位の離散化を前提にしていることに着目し、予測確率を`roundToGrid`でnBins刻みの格子点に丸めてから信頼度曲線・分解の両方に使う設計にした。これにより「ビン内で予測が完全に一定」という条件が保証され、BS=信頼度エラー-Refinement+不確実性が近似ではなく浮動小数点誤差レベルで厳密に成立する(Vitestで手計算した3項すべてを直接検算)——CLAUDE.md「数式を誤魔化さない」を、ビン平均を使う近似的な慣行ではなく厳密な恒等式で満たす判断をした。**CalibrationLabとCostLossLabで合成データセットとconfidenceScaleを共有** — 別々のLabにすると「較正の良し悪しが確率予測の価値にどう影響するか」という本トピックの核心的な接続(SPEC/Issueで要求された「確率予測がどれだけ意思決定の価値を生むか」)が見えなくなるため、あえて2つのLabを同じストアのconfidenceScale/predicted配列に依存させ、片方を動かすともう片方の価値スコアが動く設計にした。**モバイル横スクロールを機械的に発見・修正** — #92で確立した「全`<details>`展開+`document.documentElement.scrollWidth`実測+可視要素のみ(katex-mathml除外)スキャン」の手法で、Level1のMurphy分解導出にある長い$$display math$$3本が実際に横スクロールを起こしていることを発見し、`\begin{aligned}`分割で解消——同じ問題が#92以降も新規トピックで再発することを確認できたため、今後も新規Derivation追加時はこのチェックを標準手順とする。**P群5件目**、進捗 67/82。次は P-6 逐次決定（前提: markov-chains, decision-analysis）へ。

## コンテンツ拡充 #94 — [P-6] 逐次決定（sequential-decision）
- [x] worktree内で作業ブランチ feat/94-sequential-decision を作成
- [x] 出典精読: Bookmemo『意思決定分析と予測の活用』第3部「決定分析の活用」第5章「逐次決定問題における予測の活用」（逐次決定問題とは何か・標準的な枠組み(時点/状態/選択肢/政策/推移確率/報酬)・動的計画法と後ろ向き帰納法の3ステップ）。前提トピック`content/topics/decision-analysis.mdx`（決定木・後ろ向き帰納法の1期版）・`content/topics/markov-chains.mdx`（マルコフ性・遷移行列）、既存term `markov-decision-process.mdx`（reinforcement-learning #78で作成済み、再利用）・`bellman-equation.mdx`・`decision-tree-analysis.mdx`（「後ろ向き帰納法」は既にこの用語のaliasとして存在、重複作成せず再利用）を精読
- [x] 計算層 `lib/stats/sequential-decision.ts`（decision-analysisの`PRIMARY_PAYOFF_MATRIX`(工場稼働×景気の利得行列)をそのまま毎期の即時報酬として再利用し、`economyTransition(persistence)`でmarkov-chains同様の対角優位な2x2遷移行列を景気(好況/不況)に与えた具体例。`backwardInduction`が有限期間MDPのBellman方程式 $V_t(s)=\max_a[r(a,s)+\gamma\sum P(s'|s)V_{t+1}(s')]$ を t=T→1 の順(V_{T+1}=0スタート)で解く動的計画法本体、`chronological`で時系列順に並べ替え、`backwardInductionCells`でステッパー用に(期,状態)の1セルずつのpost-order列に平坦化、`expectedTotalValue`で初期分布での期待総価値を計算) + Vitest 12（period=1の近視眼的最適化、discount=1・吸収的遷移での単純合計、discount=0.9・非自明な遷移行列での手計算値との一致、計算順序、エラー系を検証）
- [x] 状態層 `lib/store/sequential-decision.ts`（`useSequentialDecisionStore`: controls={persistence, discount, periods, selectedCell}。PolicyLab/MdpLabが共有。BackwardInductionStepper用に空controlsの`useBackwardInductionStepperStore`を分離、tasks/lessons.md「複数StepPlayer共存時はストアを分離」を踏襲）
- [x] 描画層 `components/topics/sequential-decision/`: PolicyLab(L0、景気の続きやすさ・割引率・期間数のスライダーで最適政策表がブラックボックスとしてリアルタイム再計算) / BackwardInductionStepper(L1、期×状態の格子を最終期(右端列)から1セルずつコマ送りで埋めていく後ろ向き帰納法、decision-analysisのDecisionTreeStepperと同じpost-order設計を格子に適用) / MdpLab(L2、格子セルをクリックするとBellman方程式のr・割引後将来価値・Vの3項がKaTeXの`\htmlId`経由でハイライト・数値連動、操作→図→数式の強連動) / SequentialDecisionQuiz(5問、後ろ向き帰納法の順序・依存関係・割引率の効果・決定木との関係・強化学習との対比) + frames.ts(buildBackwardInductionFrames) + Vitest 5
- [x] 用語ノード新規作成: `dynamic-programming.mdx`(動的計画法、最適性原理。decision-tree-analysis/markov-decision-process/bellman-equation/value-functionのseeAlsoに相互リンク追加)。「後ろ向き帰納法」は`decision-tree-analysis.mdx`に既存aliasとして存在したため新規作成せず`<Term id="decision-tree-analysis">`で再利用。`policy.mdx`のaliasesにBookmemoの表記「政策」を追加(既存の「方策」と同義語として)
- [x] content/topics/sequential-decision.mdx（ReaderGuide + L0-2フル + L3-6 planned。decision-analysisの決定木の2ルール(確率ノード=期待値・決定ノード=最大値)をそのまま複数期間へ拡張する導出、高校数学の漸化式$a_{n+1}=f(a_n)$との対比で「なぜ最終期から解けるか」を橋渡し、決定分析のΘ(コントロールできない自然の状態)の定義から「行動が状態遷移に影響しない」という本トピックの設定が一般のMDPの自然な特殊ケースであることを導出。L2でreinforcement-learningへ「Pが既知なら動的計画法、未知ならモデルフリーな強化学習」という発展的な実リンクを追加。markov-chains・decision-analysisへ前提リンク）
- [x] 前方参照リンクの実リンク化(#93と同じ「planned枠のプレースホルダを実リンクに更新」パターン): `content/topics/decision-analysis.mdx`(L5)・`content/topics/utility-theory.mdx`(L5)・`content/topics/probabilistic-forecasting.mdx`(L6)・`content/topics/value-of-information.mdx`(L5)の「逐次決定(P-6)」プレーンテキスト/誤リンクを`/topics/sequential-decision`への実リンクに修正。`content/topics/reinforcement-learning.mdx`(L2「モデルフリー」の解説)の「動的計画法（価値反復）」を`/topics/sequential-decision`への実リンクに更新(既存の公開済みファイルなので`Edit`でピンポイント修正、`prettier --write`はかけない)
- [x] 検証(vitest 1686 / tsc / lint / build / Playwright実機)
- [x] backlog ✅done、todo レビュー追記

#### レビュー: 逐次決定トピック（2026-07-26）
- **変更概要**: P群6件目（最終）・前提markov-chains(L-1、遷移行列)+decision-analysis(P-1、決定木・後ろ向き帰納法)。核心的なアイデアは「decision-analysisの利得行列(工場稼働×景気)をそのまま毎期の即時報酬として再利用し、景気(状態)をmarkov-chainsと同じ遷移行列で期をまたいで確率的に動かす」という具体例——これにより2つの前提トピックが1つの逐次決定問題として自然に統合され、「決定木の後ろ向き帰納法は1期版、本トピックはそれをT期に一般化したもの」という関係が具体例レベルでも一貫する設計にした。PolicyLab(L0)はブラックボックスとして最適政策表を見せるだけ、BackwardInductionStepper(L1)は期×状態の格子を最終期(右端)から1コマずつ埋めていく様子をdecision-analysisのDecisionTreeStepperと同じpost-orderのコマ送りで再現し、MdpLab(L2)はマルコフ決定過程として定式化した上でセルクリック→Bellman方程式のr・割引後将来価値・Vの3項がリアルタイムにハイライト連動する操作を提供した。
- **検証結果**: Vitest **1686 passed**（+18: sequential-decision計算層12(discount=1・吸収的遷移での単純合計、discount=0.9・非自明な遷移行列での手計算値`V_1(好況)=1222`等との一致を含む)・frames 5、既存166ファイル全て回帰なし、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ、react-hooks/exhaustive-depsの警告は`cellPeriod`/`cellStateIndex`をuseEffect外で先に分離して解消）。build成功(450ページ、新規`/topics/sequential-decision`・新規`/terms/dynamic-programming`いずれも生成)。`pnpm build`後の`pnpm start`+Playwright実機確認: PolicyLabの期間数スライダーをT=5・割引率γ=0.6にすると政策表が5行に増え`t=1`のV値が連動して再計算されることを確認。BackwardInductionStepperの「1コマ後へ」を2回押すとt=5(最終期)・不況セルが「V=300, 1台稼働」で確定し、コールアウト本文の即時報酬Q値`[0台稼働:-100、1台稼働:300、2台稼働:-300]`が手計算と一致することを確認。MdpLabで格子セル(t=2,不況)をクリックすると`#term-r .katex-html`=300・`#term-future .katex-html`=876.19・`#term-v .katex-html`=826(300+0.6×876.19=825.7≈826)が連動することを確認。演習クイズの正解ボタンで「正解!」フィードバックを確認。/topics一覧に本トピックが表示されることを確認。コンソールエラー・警告0件。
- **設計判断 / つまずき**: **decision-analysisの利得行列をそのままimportして再利用** — 新規に別の利得行列を作らず`PRIMARY_PAYOFF_MATRIX`を直接importすることで、「決定木の1期版」と「逐次決定のT期版」が同じ数値で厳密に接続する(T=1のときV_1(s)=decision-analysisのMaximax相当の近視眼的最適化と一致することをVitestで確認)。**モバイル横スクロールが2箇所で発覚** — tasks/lessons.mdの既知パターン(#92)通り、Level1のBellman方程式導出で`\underbrace{}_{}`3連発の$$display math$$がモバイル390pxで実際にscrollWidth=444px(差54px)まではみ出しているのをPlaywrightで発見。地の文で既に各項の意味を説明済みだったため`\underbrace`ラベルを削って簡潔な式に置き換えたところ差44pxまで改善したが解消しきれず、2本目の`V_T(s)=\max_a[\cdots]=\max_a r(a,s)`という2段等式も同様の犯人と判明し`\begin{aligned}`で2行に分割してようやく差0pxまで解消——**1箇所直して即終了とせず、同じLevel内の残り全ての$$display mathを再スキャンする**必要があることを再確認した(教訓をtasks/lessons.mdに追記)。またビルド後の動作確認で、サーバプロセスの再起動時に古いプロセスがポートを掴んだまま残り(`pkill`のパターンマッチが効かず)、`pnpm build`で直した内容が反映されないまま古い応答を見てしまう罠に遭遇——`lsof -i :<port>`でPIDを直接特定し`kill -9`する方が確実、という運用上の教訓も得た。**P群完結**（P-1決定分析〜P-6逐次決定、全6トピック）、進捗 68/82。

## コンテンツ拡充 #95 — [Q-1] ニューラルネットワークの仕組み（neural-network-basics）
- [x] worktree内で作業ブランチ feat/95-neural-network-basics を作成
- [x] 前提トピック `content/topics/optimization.mdx`（勾配降下法・学習率・収束条件）・`content/topics/learning-framework.mdx`（経験誤差・汎化誤差・過学習・バイアス分散分解）を精読し重複を回避。既存の用語ノード `content/terms/gradient-descent.mdx` を新規作成せず再利用。Bookmemoに専門書がないため標準的な教科書レベルの正確性（連鎖律による誤差逆伝播の導出、シグモイドの勾配消失の数式的説明）を導出込みで担保する方針とした
- [x] 計算層 `lib/stats/neural-network-basics.ts`（活性化関数 ReLU/シグモイド/RBF とその微分、入力2・隠れ2・出力1の小さなネットワークの`forwardPass`/`backwardPass`(連鎖律)/`sgdUpdate`、勾配消失の`cumulativeGradientProfile`、`dropoutMask`/`applyDropout`(インバーテッド)、`batchNormalize`）+ Vitest 23（**誤差逆伝播の解析的勾配を中心差分の数値微分と比較し全パラメータ・全活性化関数(relu/sigmoid/rbf)で一致することを検証**——CLAUDE.md「数式を誤魔化さない」の実装的な担保。シグモイドの累積勾配が$0.25^k$と厳密一致、ReLUは活性域で1のまま縮まないことも確認）
- [x] 状態層 `lib/store/neural-network-basics.ts`（`useNeuralNetworkBasicsStore`: controls={x0,x1,y,activation,actX,depth,dropoutRate,bnGamma,bnBeta}。optimization.ts/learning-framework.tsと同じく単一ストアをForwardLab/ActivationLab/BackpropStepper/DropoutBatchNormLabで共有し、ステッパーは1つのみのため専用ストアを分離せず共有`frame`を使用（tasks/lessons.md #76の「ステッパーが2つ以上になった時点で分離」の判断基準に従った）
- [x] 描画層 `components/topics/neural-network-basics/`: NetworkDiagram（入力2/隠れ2/出力1/損失のSVG図、ForwardLabとBackpropStepperで再利用する共通コンポーネント）/ ForwardLab(L0、入力とシナプス結合の重み付き和→活性化関数→出力を数式と図で強連動)/ ActivationLab(L1、ReLU・シグモイド・RBFの曲線と微分の比較+勾配消失チェーンの棒グラフ)/ BackpropStepper(L2、計算グラフを順伝播5コマ→逆伝播4コマでコマ送り、`useFramePlayer`配線済み)/ DropoutBatchNormLab(L2、ドロップアウトマスクとバッチ正規化を数値で体感)/ NeuralNetworkBasicsQuiz(6問) + frames.ts(buildBackpropFrames) + Vitest 6
- [x] 用語ノード新規作成11件: `neural-network-layer.mdx`(入力層・隠れ層・出力層)・`synaptic-connection.mdx`(シナプス結合)・`activation-function.mdx`・`relu.mdx`・`sigmoid-function.mdx`(logistic-regression.mdxとは別ノードとして新設、相互seeAlso)・`radial-basis-function.mdx`・`backpropagation.mdx`(連鎖律4段の導出込み)・`vanishing-gradient.mdx`(層数$L$の累積勾配$d^L$の数式込み)・`dropout.mdx`・`batch-normalization.mdx`・`computational-graph.mdx`。全てregistry.test.tsでリンク切れ0を確認
- [x] `content/topics/neural-network-basics.mdx`（ReaderGuide + L0ブラックボックス(順伝播)/L1古典的理解(活性化関数比較+シグモイド微分の完全導出)/L2仮定体系(誤差逆伝播の連鎖律4段導出+ドロップアウト・バッチ正規化+確率的勾配降下法の説明)/L3-L6 planned(アーキテクチャ・最適化の工夫・表現学習理論・フロンティア)。前提optimization・learning-frameworkへ実リンク。トピック本文の用語参照は全て`<Term id>`（裸のmarkdownリンクは使用せず、#79の教訓を踏襲）
- [x] 前方参照の実リンク化: `content/topics/learning-framework.mdx`(L5「ニューラルネットを」)・`content/topics/reinforcement-learning.mdx`(前提節「Qテーブルをニューラルネットワークに」)のプレーンテキストを`/topics/neural-network-basics`への実リンクに更新
- [x] 検証(Vitest 1715 / tsc / lint / build 462ページ / Playwright実機・モバイル390px)
- [x] backlog ✅done、todo レビュー追記

#### レビュー: ニューラルネットワークの仕組みトピック（2026-07-26）
- **変更概要**: Q群1件目。入力2・隠れ2・出力1の最小構成ネットワーク1つを全Levelで使い回す設計にした——ForwardLab(L0)で順伝播を体感し、ActivationLab(L1)で活性化関数の選択がBackpropStepper(L2)の逆伝播にそのまま影響することを、同じ`NET_PARAMS`・同じcontrols(x0,x1,y,activation)を共有することで一貫させた。核心は誤差逆伝播の数値的正しさを**単体テストで解析的勾配と数値微分(中心差分)を突き合わせて証明した**こと——backpropagation.mdxやトピックL2の導出が「机上の空論」でなく実装と厳密に一致することを保証する。勾配消失の可視化(ActivationLabの累積勾配バー)はシグモイドで$0.25^k$と厳密一致する数値を見せ、ReLUとの対比を数式・グラフ・棒グラフの3方向から示した。
- **検証結果**: Vitest **1715 passed**（+29: neural-network-basics計算層23(誤差逆伝播の解析的勾配=数値微分の一致を全パラメータ×全活性化関数で検証、シグモイド累積勾配$0.25,0.0625,\dots$の厳密一致、ドロップアウトの期待値保存の統計的検証を含む)・frames 6、既存168ファイル全て回帰なし、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ）。`pnpm build`成功(462ページ、新規`/topics/neural-network-basics`・新規用語11件いずれも生成)。`pnpm build`後の`pnpm start`+Playwright実機確認: ForwardLabのx₀スライダーをx₀=2に動かすと`#nn-forward`の数式表示が`z₁⁽⁰⁾=0.95, a₁⁽⁰⁾=0.72, ŷ=0.27`のように手計算と一致して連動することを確認。BackpropStepperの「1コマ後へ」を2回押すとステップ3/9(隠れユニット2)のコールアウトが同じx₀=2の値を反映して更新されること、「▶ 再生」を押すと`useFramePlayer`が実際に約3秒でコマ3→7まで自動進行することを確認。ActivationLabで活性化関数をReLU→シグモイドに切り替え層数を6にすると、累積勾配バーの表示値が`1,0.25,0.063,0.016,0.004,9.8e-4,2.4e-4`（$0.25^k$と厳密一致）に変わることを確認。DropoutBatchNormLabのdrop率スライダーを0.5にすると各ユニットが決定的なシード付き乱数で「残す(1/(1−0.5)倍)」「落とす(0)」に振り分けられることを確認。全11用語ページ(`/terms/relu`等)がHTTP 200を返すこと、Term参照の赤波線(リンク切れ表示)が0件であることを確認。コンソールエラー・警告0件。太字`**`残存0件。
- **設計判断 / つまずき**: **モバイル横スクロールの新パターンを発見** — L1のシグモイド微分導出で、1行に`=`で4項をつないだ長い`$$...$$`(`\boxed`ではなくその直前の連立導出式)がモバイル390pxで実際にscrollWidth=623px(差233px、可視要素のオーバーフローで#84の許容範囲を大きく超える)まではみ出しているのをPlaywrightで発見。同型の`\boxed{}`単体は既存公開トピック(learning-framework.mdx)との比較で差49px(#84許容範囲)と分かり、`\boxed`自体は無罪で「3項以上を`=`で連結した1行の連立式」が真因と特定——`\begin{aligned}`で各等号ごとに改行して差0pxまで解消した。この「連立等式は項数が増えるほど1行の総幅が線形に伸びる」というパターンは#92のunderbrace型とは別の新しいはみ出し要因のため、tasks/lessons.mdに追記した。**Q群開始**（Q-1ニューラルネットワークの仕組み完了、Q-2 NNモデル(CNN/RNN)以降が続く）、進捗 69/82。

## コンテンツ拡充 #96 — [Q-2] NNモデル（CNN・RNN）（neural-network-models）
- [x] worktree内で作業ブランチ feat/96-neural-network-models を作成（worktreeは事前準備済み、origin/mainは#95まで反映済みを確認）
- [x] 前提トピック `content/topics/neural-network-basics.mdx` を精読し、活性化関数・誤差逆伝播・計算グラフ・勾配消失などの既出概念を重複説明せず参照。既存 `content/terms/convolution.mdx`（確率論の畳み込み＝独立確率変数の和の分布）とCNNの畳み込み層は別概念と確認し、新規`convolutional-layer.mdx`を作成（両者の関係はconvolutional-layer側の本文で軽く言及しseeAlsoでも接続、混同を避ける記述に）
- [x] 計算層 `lib/stats/neural-network-models.ts`: CNN側=`convOutputSize`(floor((n+2p-f)/s)+1)/`padMatrix`/`convWindowSum`/`conv2d`/`windowPositions`/`pool2d`(max/avg)、RNN側=`tanhDerivative`/`rnnStep`/`rnnUnroll`/`rnnOutput`/`rnnGradientProfile`(時間方向BPTT)、LSTM側=`lstmStep`/`lstmUnroll`/`lstmCellGradientProfile`(セル状態を通る勾配=Π忘却ゲート) + Vitest 27（**RNN/LSTMいずれも勾配プロファイルを中心差分の数値微分と突き合わせて検証**——LSTMは忘却ゲートがh依存しない特殊ケース(u*=0)で厳密一致、さらに「忘却ゲートを1近くに保てるLSTMは同条件のRNNより勾配が緩やかにしか縮まない」という核心的主張を実測比較で検証。conv2d/pool2dは手計算例と一致、出力サイズ公式はpadding/stride複数ケースで検証）
- [x] 状態層 `lib/store/neural-network-models.ts`（`useNeuralNetworkModelsStore`: CNN controls={filterPreset,convStride,convPadding,windowIndex,poolMode,poolStride}・RNN controls={rnnMode,seqT,forgetBias}・転移学習controls={frozenLayers}を1ストアに集約。ステッパーが2つ(ConvPoolStepper・SequenceStepper)のため、tasks/lessons.md #76に従い`useConvPoolFrameStore`/`useSequenceFrameStore`を専用の空ストアとして分離）
- [x] 描画層 `components/topics/neural-network-models/`: Grid(CNN共通グリッド表示部品)/ ConvLab(L0、フィルタ・ストライド・パディングと着目窓の実時間操作→数式強連動)/ ConvPoolStepper(L1、畳み込み9コマ→プーリング4コマの計13コマをコマ送り、入力窓・出力マスを色ハイライト)/ RnnLstmLab(L0、RNN⇄LSTM切替は同一`<MathFormula>`のtex差し替えで実装——tasks/lessons.md #79の教訓どおり条件付きマウント切替を回避、忘却ゲートバイアスのスライダーで勾配プロファイル棒グラフが実時間で伸縮)/ SequenceStepper(L2、系列5ステップをコマ送り、LSTMモードはゲート値を赤(閉)〜緑(開)で色分け)/ TransferLearningDiagram(L2、5層の凍結/再学習をクリックトグルする簡易概念図)/ NeuralNetworkModelsQuiz(7問) + convPoolFrames.ts/sequenceFrames.ts(純関数フレームビルダー) + Vitest 12
- [x] 用語ノード新規作成6件: `convolutional-layer.mdx`（畳み込み層/CNNフィルタ、出力サイズ公式込み、confusionを避けるためconvolution.mdxとの関係を明記）・`pooling-layer.mdx`・`recurrent-neural-network.mdx`（BPTTの勾配消失を数式で導出）・`lstm.mdx`（3ゲート+セル状態の全式、∂c_t/∂c_{t-1}=f_tの導出込み）・`gate-mechanism.mdx`・`transfer-learning.mdx`。全てregistry.test.tsでリンク切れ0を確認
- [x] `content/topics/neural-network-models.mdx`（ReaderGuide + L0ブラックボックス(CNN/RNNをそれぞれ最小の式で導入、ConvLab+RnnLstmLab)/L1古典的理解(出力サイズ公式の「窓を置ける位置を数える」高校数学的導出+RNNのBPTT連鎖律導出、ConvPoolStepper)/L2仮定体系(CNNの並進不変性という仮定と崩れる例、LSTMセル状態の加法的勾配経路の導出、転移学習、SequenceStepper+TransferLearningDiagram+Quiz)/L3-L6 planned(画像解析/テキスト解析は別issueへ委譲する旨を明記、Transformer等はQ-5へ)。前提neural-network-basicsへ実リンク
- [x] 検証（Vitest 1754 / tsc 0エラー / lint新規警告0 / build 469ページ / Playwright実機・モバイル390px・デスクトップ1400px）
- [x] backlog ✅done、todo レビュー追記

#### レビュー: CNN・RNNトピック（2026-07-26）
- **変更概要**: Q群2件目。全結合層（Q-1で確立）を「データの構造に応じてどう特殊化するか」という一貫した軸でCNN（空間方向のパラメータ共有）とRNN/LSTM（時間方向の再帰）を並べて扱った。核心は2つ: (1) CNNの出力サイズ公式を「窓を置ける位置を数える」という高校数学の等差数列の考え方で導出し、数式の結論だけを示さない（CLAUDE.md §0-4）。(2) LSTMがなぜ勾配消失に強いかを「セル状態の更新が掛け算でなく足し算の経路を持つ」という数式的な理由（∂c_t/∂c_{t-1}=f_t）まで導出し、RNNの時間方向BPTT（層方向の勾配消失と同型）と対比させた——この主張は`lstmCellGradientProfile`のテストで、ゲートが隠れ状態に依存しない特殊ケース（u*=0）において数値微分と厳密一致することを検証し、さらに「忘却ゲートを開けたLSTMは同条件のRNNより勾配が長く残る」という定性的主張も実測の比較テストで裏付けた（tasks/lessons.mdの「近似手法は解析解を知っている題材で検証する」#86の方針を踏襲）。
- **検証結果**: Vitest **1754 passed**（+39: neural-network-models計算層27・convPoolFrames 7・sequenceFrames 5、既存171ファイル全て回帰なし、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ、修正2件: 未使用importと未使用変数）。`pnpm build`成功(469ページ、新規`/topics/neural-network-models`・新規用語6件いずれも生成、`/topics`一覧にも反映確認)。`pnpm build`後の`pnpm start`+Playwright実機確認: ConvLabで「次の窓へ」を押すと`z_{1,1}=-20`→`z_{1,2}=-8`のように数式のハイライト値が特徴マップの実際の値と一致して連動、パディングを1に変えると出力サイズ公式の表示値が`3`→`5`に即座に更新されることを確認。ConvPoolStepperで「1コマ後へ」を3回・「▶再生」を約3秒でステップが自動進行することを確認（tasks/lessons.md #80のuseFramePlayer配線漏れがないことの実機証拠）。RnnLstmLabでRNN⇄LSTM切替・時刻tスライダー・忘却ゲートバイアススライダーが数式とグラフに即座に反映されることを確認。SequenceStepperをLSTMモードにし系列を進めると、x=1.5(t=1)で忘却ゲート0.86(緑)、x=-1.8(t=4)で忘却ゲート0.25(赤)と符号に応じて実際にゲートが開閉する色変化を確認。TransferLearningDiagramの層クリックで凍結⇄再学習が切り替わることを確認。Quizで正解選択時に緑の解説が表示されることを確認。全6用語ページがHTTP 200、太字`**`残存0件（6用語+トピックページ全てで確認）。モバイル390px（`<details>`全展開）でscrollWidth差10px（#84許容範囲内）、可視要素のオーバーフロー0件。コンソールエラー・警告0件（全ページ）。
- **設計判断 / つまずき**: **モバイル横スクロールの新パターンを発見（#92/#95の「連立等式」「\underbrace」とは別種）**——L0で「CNN: 畳み込み層」「RNN: 再帰計算」という日本語ラベルを`\qquad(\text{...})`で数式内に付けたところ、単独では短く見える数式でもモバイル390pxで実際にscrollWidth差64px（可視要素のオーバーフローで確認、`.katex-mathml`起因ではない）まではみ出すことをPlaywrightで発見。地の文（直後の`<Term>`付き説明文）で既に同じ内容を説明していたため、数式内の`\text{}`注釈を削除（#92の対策方針「地の文で既に説明している情報を数式側でも重複して持たせている場合は削る」を実践）して差10pxまで解消。**新知見**: MDX本文の生の`$$...$$`（`<MathFormula>`のような自前のoverflow-x-autoラッパーを持たない）は、短い数式に見えても`\text{日本語}`を含むと`cjk_fallback`フォントで幅を大きく消費し、モバイルで見た目にもはみ出しうる——tasks/lessons.mdに追記した。**Q群2件目完了**（Q-3テキスト解析・Q-4画像解析が次、それぞれ本トピックで深掘りしなかったテキスト/画像応用の詳細を担当）、進捗 70/82。

## コンテンツ拡充 #97 — [Q-3] テキスト解析（text-analysis）
- [x] worktree内で作業ブランチ feat/97-text-analysis を作成
- [x] 前提トピック `content/topics/neural-network-basics.mdx` を精読。単語埋め込みを埋め込み層としてNNと接続する箇所（L2・ReaderGuide）で活性化関数・シナプス結合・勾配降下法の既出概念を再利用し重複説明しない方針とした。対象キーワード10件（形態素解析・tf-idf・ステミング・ストップワード・単語埋め込み・ベクトル空間モデル・共起ネットワーク・トピックモデル・言語モデル・n-gram）に対応する用語ノードが未作成であることを確認し全て新規作成
- [x] 計算層 `lib/stats/text-analysis.ts`（純関数）: 分かち書き（辞書ベース前方最長一致法`greedySegment`/`candidatesAt`）、前処理（`tokenize`/`removeStopWords`/`stem`（接尾辞ルールベース、見出し語化との違いを`programming→programm`で明示）/`preprocessSentence`/`preprocessCorpus`）、tf-idf（`termFrequency`/`documentFrequency`/`inverseDocumentFrequency`/`tfIdf`/`tfIdfMatrix`）、n-gram言語モデル（`ngrams`/`countNgrams`/`conditionalProbability`（ラプラススムージング）/`topNextWords`/`sentenceProbability`（連鎖律の数値確認）/`contextOptions`）、共起（`coOccurrenceCounts`/`coOccurrenceMatrix`）、ベクトル演算（`dot`/`vecNorm`/`cosineSimilarity`）、単語埋め込み・ベクトル空間モデル（`wordVectors`（共起行列そのものをカウントベース埋め込みとして再利用）/`queryVector`/`rankDocumentsByQuery`）、簡易トピックモデル（`farthestPairInit`（乱数を使わず最もコサイン類似度が低い2文書を選ぶ決定的な初期化）/`assignByNearestCentroid`/`updateCentroidsMean`/`runSimpleTopicModel`/`topWordsPerTopic`）+ Vitest 37（tf-idf・共起カウント・n-gram確率は手計算値（例: P(science|data)=5/26、combineのtf-idf=0.2×ln5=0.3219）と厳密一致することを確認、トピックモデルは決定性（同一入力→同一出力）を検証）
- [x] 状態層 `lib/store/text-analysis.ts`（`createTopicStore`パターン。controls={jaSentenceIndex, enSentenceIndex, ngramN, ngramContextIndex, windowSize, wordAIndex, wordBIndex, queryPresetIndex}。ステッパーが3つ（MorphSegmentStepper・PreprocessStepper・TopicModelStepper）のため、tasks/lessons.md #76に従い`useMorphFrameStore`/`usePreprocessFrameStore`/`useTopicModelFrameStore`を専用の空ストアとして分離）
- [x] 描画層 `components/topics/text-analysis/`: MorphSegmentStepper（L0、前方最長一致法の走査をコマ送り、選択中の文字範囲を色ハイライト）/ PreprocessStepper（L0、トークン化→ストップワード除去→ステミングの3フェーズをコマ送り）/ TfidfLab（L1、語クリック→tf・idf・tf-idfが数式と連動）/ NgramLab（L1、n・文脈選択→次語確率の棒グラフと数式が連動）/ CooccurrenceNetworkLab（L2、窓幅スライダー＋ノードクリックで近傍だけハイライト＝文字通りの近傍コールアウト）/ VectorSpaceLab（L2、単語埋め込みのコサイン類似度と文書検索を同じ道具で並べる）/ TopicModelStepper（L2、k=2クラスタリングの反復をコマ送り）/ TextAnalysisQuiz（10問、10キーワード網羅）+ morphFrames.ts/preprocessFrames.ts/topicModelFrames.ts（純関数フレームビルダー）+ Vitest 8
- [x] 用語ノード新規作成10件: `morphological-analysis.mdx`・`stop-words.mdx`・`stemming.mdx`（見出し語化との対比表込み）・`tf-idf.mdx`（idfの対数を使う理由の導出込み）・`n-gram.mdx`・`language-model.mdx`（連鎖律からn-gram近似への導出込み）・`co-occurrence-network.mdx`・`word-embedding.mdx`（synaptic-connection・gradient-descentへリンクしNNの埋め込み層と接続）・`vector-space-model.mdx`・`topic-model.mdx`（本格的なLDAとの違いを明記）。全てregistry.test.tsでリンク切れ0を確認
- [x] `content/topics/text-analysis.mdx`（ReaderGuide + L0ブラックボックス(前処理パイプライン: 分かち書き→ストップワード除去→ステミング)/L1古典的理解(tf-idfのidf対数の導出+n-gram連鎖律近似の導出)/L2仮定体系(分布仮説を軸に共起ネットワーク・単語埋め込み・ベクトル空間モデル・トピックモデルを統一、コサイン類似度の余弦定理からの導出)/L3-L6 planned(分散表現・応用タスク・大規模言語モデル・フロンティア、機械翻訳はSPEC記載の発展項目として言及のみ)。前提neural-network-basicsへ実リンク
- [x] 検証（Vitest 1799 / tsc 0エラー / lint新規警告0 / build 480ページ / Playwright実機・モバイル390px）
- [x] backlog ✅done、todo レビュー追記

#### レビュー: テキスト解析トピック（2026-07-26）
- **変更概要**: Q群3件目。古典的テキスト解析パイプライン全体（前処理→頻度ベース表現→ベクトル空間表現）を、5文の小さな英語コーパス＋日本語の分かち書きデモという一貫した具体例で通した。核心は2つ: (1) tf-idfのidfになぜ対数を使うかを「df=Nで0になる」「対数で伸びを穏やかにする」という2つの望ましい性質から導出し、n-gram言語モデルは確率の連鎖律から「直前n-1語で近似する」というマルコフ的仮定への導出をL1のDerivationで明示（CLAUDE.md §0-4、数式の結論だけを書かない）。(2) L2「仮定体系」を分布仮説（意味の近い語は似た文脈に現れる）1つに統一し、共起ネットワーク・単語埋め込み（共起カウントをそのままベクトルにする簡易版）・ベクトル空間モデル（tf-idfベクトル+コサイン類似度による文書検索）・簡易トピックモデル（コサイン類似度ベースのk=2反復クラスタリング、乱数を使わず最も似ていない2文書を初期中心にする決定的な設計）が「対象をベクトルにしコサイン類似度で近さを測る」という同じ道具立ての応用であることを一貫して示した。トピックモデルは本格的なLDAのギブスサンプリングではなく簡略計算である旨を用語ノード・トピック本文の両方で明記し誤解を避けた。
- **検証結果**: Vitest **1799 passed**（+45: text-analysis計算層37（P(science|data)=5/26、combineのtf-idf=0.2×ln5≈0.3219、共起カウント`data|science`=4など手計算値と厳密一致）・morphFrames/preprocessFrames/topicModelFrames各テスト、既存175ファイル全て回帰なし、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ）。`pnpm build`成功(480ページ、新規`/topics/text-analysis`・新規用語10件いずれも生成、`/topics`一覧にも反映確認)。`pnpm build`後の`pnpm start`+Playwright実機確認: MorphSegmentStepperで「1コマ後へ」を押すと「データサイエンス」（8文字・辞書の最長一致）→「が」の順に確定しステップ表示が2/3へ進むことを確認。TfidfLabで「data」をクリックすると`#term-tf`=0.2・`#term-idf`=0.223・`#term-tfidf`=0.0446と手計算通りに数式が連動することを確認。NgramLabの初期文脈を「data」にしたところ`P(science|data)=0.1923`（=5/26の実測値）が数式・棒グラフ双方に反映されることを確認。CooccurrenceNetworkLabで「data」ノードをクリックすると近傍コールアウトに「science(×4)」を含む8語が表示されることを確認。TopicModelStepperで反復1回目にD1(combine)・D4(project/skill/support)がトピックB、D2/D3/D5(machine/learn中心)がトピックAに分かれる、意味のある挙動を確認。全10用語ページがHTTP 200、Term参照の赤波線（リンク切れ表示）0件。モバイル390px（`<details>`全展開）でトピックページのscrollWidth差4px（#84許容範囲内）、10用語ページ全てで差0px。コンソールエラー・警告0件（全11ページ）。
- **設計判断 / つまずき**: **`<select>`の閉じたボックス幅が最長optionテキストに引っ張られてモバイル横スクロールする新パターンを発見**——PreprocessStepper/TfidfLabの文書選択セレクトに`D1: data science combines statistics and programming`のような長い英文全体をoption文言に入れていたところ、Chromiumが`<select>`要素自体の幅を最長optionに合わせて広げてしまい、モバイル390pxでscrollWidth差93pxの実害（`.katex-mathml`起因ではなく`<select>`本体が犯人）が発生。option文言を`D1`のような短いラベルに変え、全文はselect横の`<span className="min-w-0 flex-1 break-words">`（flexアイテムの既定`min-width:auto`を打ち消すため`min-w-0`必須——付け忘れるとテキストが折り返されず同じ横スクロールが再発する）で別途表示する形に修正して解消。既存の`\underbrace`/連立等式/`\text{日本語}`パターン（#92/#95/#96）とは異なる「`<select>`自体の内在幅」という第4のはみ出し要因のため、tasks/lessons.mdに追記した。**用語ページ（`/terms/[slug]`）もトピックページと同じくbare`$$...$$`が自前のoverflow-x-autoラッパーを持たないため同じ横スクロール対策が必要**と判明——tf-idf.mdx（2つの計算例をqquadで1行に連結）・n-gram.mdx（3つ組のタプルを1行に連結）・language-model.mdx・vector-space-model.mdxの計4ファイルで同様に数式を複数行へ分割し、既存の40件超の公開済み用語ページ（backpropagation.mdx等）にも同種の`\text{日本語}`パターンが多数残っていることを確認したが、#84の既存判断（サイト横断のスコープ外）を踏襲し本Issueでは自分が新規作成した10件のみ対処した。**Q群3件目完了**（Q-4画像解析が次）、進捗 71/82。

## コンテンツ拡充 #98 — [Q-4] 画像解析（image-analysis）
- [x] worktree内で作業ブランチ feat/98-image-analysis を作成
- [x] 前提トピック `content/topics/neural-network-models.mdx` と用語ノード `convolutional-layer.mdx`/`pooling-layer.mdx`/`transfer-learning.mdx` を精読。CNNの畳み込み・プーリングの計算メカニズム自体は既習として再導出せず、画像処理・画像認識・物体検出という応用面に焦点を当てる方針を確認。対象キーワード3件（画像処理・画像認識/分類・物体検出）に対応する用語ノードが未作成であることを確認し、関連語（ソフトマックス・バウンディングボックス・IoU・NMS・アンカーボックス）も含め全て新規作成
- [x] 計算層 `lib/stats/image-analysis.ts`（純関数）: 画像処理（`padMatrix`/`convWindowSum`/`conv2d`/`windowPositions`、Sobel`SOBEL_X`/`SOBEL_Y`、平均化`AVERAGE_BLUR_3`、`gradientMagnitude`(√(Gx²+Gy²))、`binarize`）、画像認識・分類（`softmax`（最大値を引く数値安定化込み）、`argmax`、`topKPredictions`）、物体検出（`boxArea`/`intersectionArea`/`unionArea`/`iou`、`nonMaxSuppression`（採用→抑制のステップをトレースとして返す）、`generateAnchors`（w=scale√ratio・h=scale/√ratioの連立方程式を解いて生成）) + Vitest 33（Sobel Xが縦エッジ境界で正・一様領域で0になることを実際に検証、softmaxの総和1・オーバーフロー非発生・最大ロジット一致を確認、NMSが2クラスタから正しく2件採用する既知シナリオと厳密一致することを確認）
- [x] 状態層 `lib/store/image-analysis.ts`（`useImageAnalysisStore`: controls={filterPreset, windowIndex, threshold, logits, boxA, boxB, nmsIouThreshold, anchorHighlightIndex}。ステッパーが2つ（EdgeDetectionStepper・NmsStepper）のため、tasks/lessons.md #76に従い`useEdgeStepperFrameStore`/`useNmsStepperFrameStore`を専用の空ストアとして分離）
- [x] 描画層 `components/topics/image-analysis/`: Grid（画像解析共通グリッド表示部品、NNモデルのGridと同型だがトピック間直接依存を避け独立実装）/ FilterLab（L0/L1、Sobel Gx・Gy・勾配強度・ぼかし・二値化の5プリセットを切替→出力グリッドと数式(z_{i,j}=ΣK·X or G=√(Gx²+Gy²) or p(x)=1[x≥t])が実時間連動）/ EdgeDetectionStepper（L1、5×5入力にSobel Xフィルタを1マスずつスライドさせる9コマのコマ送り）/ ClassificationLab（L0/L1、3クラス(猫・犬・鳥)のロジットスライダー→softmax確率バー+数式(P(y=k|x)=e^zk/Σe^zj)が連動）/ IoULab（L0/L1、2つのバウンディングボックスをSVGドラッグ→共通部分・和集合・IoUが数式と連動、pointer capture方式のドラッグ実装）/ NmsStepper（L2、5候補ボックス(2物体×重複検出)からNMSが採用/抑制を確定させる2コマのコマ送り）/ AnchorBoxDiagram（L2、中心点からスケール×縦横比の6アンカーを生成し「次へ」で着目アンカーの幅・高さの連立方程式の解が数式と連動）/ ImageAnalysisQuiz（8問、8キーワード網羅）+ edgeFrames.ts/nmsFrames.ts（純関数フレームビルダー）+ Vitest 13
- [x] 用語ノード新規作成9件: `image-processing.mdx`・`edge-detection.mdx`（Sobel Gx/Gy+勾配強度の三平方の定理での合成）・`image-classification.mdx`・`softmax-function.mdx`（2クラスでロジスティック回帰のシグモイドと一致することの導出込み）・`object-detection.mdx`・`bounding-box.mdx`・`intersection-over-union.mdx`（包除原理での和集合面積の導出）・`non-maximum-suppression.mdx`・`anchor-box.mdx`（w=scale√ratio, h=scale/√ratioの連立方程式の導出）。全てregistry.test.tsでリンク切れ0を確認
- [x] `content/topics/image-analysis.mdx`（ReaderGuide + L0ブラックボックス(画像処理/画像認識/物体検出の入出力の違いをFilterLab+ClassificationLab+IoULabで一望)/L1古典的理解(勾配強度の三平方の定理での導出+ソフトマックスの指数関数を使う理由の導出(2クラスでシグモイドと一致することも導出)+IoU分母の包除原理での導出、EdgeDetectionStepper+FilterLab)/L2仮定体系(NMSの貪欲アルゴリズムという設計・アンカーボックスの「典型的な物体形状に絞り込む」という仮定とその限界、アンカー生成の連立方程式導出、NmsStepper+AnchorBoxDiagram+Quiz)/L3-L6 planned(R-CNN系/YOLO系・セグメンテーション、IoU損失等の学習応用、Grad-CAM等の解釈可能性、ViT/拡散モデル等のフロンティア)。前提neural-network-modelsへ実リンク
- [x] 検証（Vitest 1845 / tsc 0エラー / lint新規警告0 / build 490ページ / Playwright実機・モバイル390px・デスクトップ1280px）
- [x] backlog ✅done、todo レビュー追記

#### レビュー: 画像解析トピック（2026-08-01）
- **変更概要**: Q群4件目（最終）。前提のNNモデル（CNN・RNN、#96）がCNNの畳み込み・プーリングという計算メカニズムそのものを既に扱っていたため、本トピックは意図的に計算メカニズムを再導出せず、3つの応用（画像処理・画像認識/分類・物体検出）に焦点を絞った。核心は3つの導出: (1) Sobelの縦・横勾配Gx・Gyを「直交する2辺」とみなし三平方の定理と同じ形（G=√(Gx²+Gy²)）で合成する導出。(2) ソフトマックスがなぜ指数関数を使うか（非負性の確保・大小関係の保存）を導出し、2クラスの特別な場合が[ロジスティック回帰](logistic-regression)のシグモイド関数と厳密に一致することまで示した。(3) IoUの分母（和集合の面積）が高校数学の包除原理`|A∪B|=|A|+|B|-|A∩B|`と同じ考え方であることの導出。L2では「NMSは貪欲アルゴリズムであり最適性を保証しない」「アンカーボックスは典型的な物体形状に絞り込むという仮定を置き、崩れる例（電柱のような極端に細長い物体）がある」という近似・仮定の限界も明記し、CLAUDE.mdの「仮定体系」レベルの趣旨を満たした。
- **検証結果**: Vitest **1845 passed**（+46: image-analysis計算層33（Sobel Xが境界窓のみ正・一様窓は0になることを実際の5×5画像で検証、softmaxの総和1・大値でのオーバーフロー非発生・空配列時の安全な挙動、NMSが2クラスタ(候補①③⑤重複あり)から正しく候補①・候補④の2件のみ採用することを検証、アンカーの面積・縦横比が連立方程式の解と一致することを検証）・edgeFrames 6・nmsFrames 7、既存178ファイル全て回帰なし、registryリンク切れゼロ）。tsc エラー0。`pnpm lint`新規ファイル警告0（既存1件の無関係warningのみ）。`pnpm build`成功(490ページ、新規`/topics/image-analysis`・新規用語9件いずれも生成、`/topics`一覧にも反映確認)。`pnpm build`後の`pnpm start`+Playwright実機確認: FilterLabのフィルタを二値化に切り替え閾値スライダーをネイティブsetterで250に動かすと、数式`#term-ia_t`=250・`#term-ia_p`=0（元画像の該当画素200<250）が連動し、出力グリッドの該当マスが0になることを確認。ClassificationLabで「鳥」のロジットスライダーを5まで上げると、softmaxバーが鳥93%まで伸び予測ラベルとCallout本文が「予測: 鳥」に切り替わることを確認。IoULabでボックスAをSVGドラッグ（scrollIntoViewIfNeeded後にPlaywrightの実マウスイベントで検証、pointer captureが正しく機能）すると、IoU値が0.185→0.016まで実時間で減少し数式と連動することを確認。EdgeDetectionStepperの「▶再生」を押し3.2秒待つとステップ1→3まで自動進行することを確認（tasks/lessons.md #80のuseFramePlayer配線漏れがないことの実機証拠）。NmsStepperで「1コマ後へ」を押すと候補④採用・候補⑤抑制のCalloutが表示されることを確認。AnchorBoxDiagramで「次のアンカーへ」を押すとscale32・ratio0.5に切り替わり`#term-ia_w`が28.28→22.63（=32×√0.5の手計算と一致）に更新されることを確認。全9用語ページ+トピックページでHTTP 200、Term参照の赤波線（リンク切れ表示）0件、太字`**`残存0件。モバイル390px（`<details>`全展開）でトピックページ・9用語ページ全てscrollWidth差0px。コンソールエラー・警告0件（全10ページ、複数回のインタラクション後も含む）。
- **設計判断 / つまずき**: **モバイル横スクロールを新たに3箇所発見・修正** — (1) L1 DerivationのIoU和集合公式`Area(A∪B)=Area(A)+Area(B)-Area(A∩B)`（3項を`=`/`-`で連結、#95の「3項以上の連立式」パターン）がscrollWidth差26pxではみ出しているのをPlaywrightで発見し`\begin{aligned}`で2行に分割。(2) L2 Derivationのアンカーボックス連立方程式で`\qquad(\text{面積の目安})`のように日本語注釈を`\qquad`で数式に付けていた箇所（#96の「\text{日本語}注釈」パターン）を、地の文で既に同じ内容を説明済みだったため削除。(3) L1 Derivationの`e^{z_1},\ e^{z_2},\ \dots,\ e^{z_K}\quad(\text{すべて正})`も同様の日本語注釈パターンで削除。同じ`Area(A∪B)=...`の3項連結式が`content/terms/intersection-over-union.mdx`にも重複して存在したため、term側も同じ`\begin{aligned}`分割で修正した——**執筆時点でこれらのパターン（#95/#96で既知）を機械的に避けたつもりでも、Derivationを複数書くと1つは必ず紛れ込む**ため、Playwright実機確認は「既知パターンを避けて書いたから大丈夫」で済ませず、必ず`scrollWidth`の実測で最終確認する必要があると再確認した。**Playwright実機確認の手順上の教訓**: SVGドラッグ操作をテストする際、`getBoundingClientRect()`は現在のビューポート基準の座標を返すため、ページが長く要素が現在のスクロール位置から外れていると（y座標がビューポート高さを大きく超える等）、その座標で`page.mouse`を操作しても要素にヒットしない——`scrollIntoViewIfNeeded()`で対象要素を可視範囲に入れてから座標を計算しないと「操作しても何も起きない」ように見える。加えて、重なり合う複数のSVG要素（本トピックのIoULabのボックスA/Bのように、片方の境界がもう片方の内部に食い込む配置）をクリック位置だけで区別する場合、クリック座標がちょうど両者の重なり境界に来ると、DOM順で後に描画された要素（手前）が意図せずヒットしうる——ドラッグ操作の実機テストでは、対象が単独で占有する領域内の座標を選ぶ。**Q群完結**（Q-1〜Q-4、全4トピック）、進捗 72/82。
