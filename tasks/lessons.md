...
## 近似手法（MCMC・変分ベイズ等）は「既に解析解を知っている題材」を目標分布にすると、正しさをその場で検証できる（出典 #86）

- **背景**: mcmc-methods（K-2）はK-1（bayesian-basics）で共役事前分布により解析的に求めたBeta(6,6)事後分布と**同じ題材**を、今回は「正規化定数を知らないふり」をしてMCMCでサンプリング近似する設計にした。もし代わりに未知の複雑な分布を目標にすると、「MCMCが本当に正しい分布を復元できているか」を読者もテストコードも直接確認する手段がない（ヒストグラムが収束したように見えても、それが正しい先か判断できない）。
- **対策**: 計算層のテストで「正規化定数抜きの核（betaTargetKernel）を使っても、正規化済みの密度（betaPdf）を使っても、同じ乱数列に対して受理判定が完全に一致する」ことを直接アサートする単体テストを追加した（`lib/stats/mcmc-methods.test.ts`）。これにより「MCMCは正規化定数を必要としない」という主張が具体的な数値の一致で証明され、かつ描画側はK-1の`betaPdfCurve`をそのまま再利用してヒストグラムとの視覚比較もできる——一次情報（正解）を持つ題材を選ぶことで、テスト・可視化・文章の3つが同時に「正しさ」を裏付け合う構成になった。
- **判断の目安**: 近似アルゴリズム系のトピック（MCMC・変分ベイズ・EM法・ブートストラップ等）を実装するときは、可能な限り「前提トピックで既に解析的に答えを出した題材」を目標分布/推定対象として再利用する。新しい題材を使う場合でも、既知の解析解を持つ簡単なケース（正規分布、共役事前分布など）を最低1つはテスト・可視化のどちらかに含め、「近似が収束した先が本当に正しいか」を検証可能にする。

## MDX太字が閉じ`**`の右フランキング規則で消える（続報）：`**$...$`のように直接`$`（インライン数式）に接すると崩れる（出典 #88）

- **症状**: 「実は**$n=a+b-1$ 個の…」のように、太字の開始直後に間を置かず `$`（KaTeXインライン数式の開始）が続く書き方で、太字が効かず `**` がそのまま画面表示される。#73/#78 で見つけた「閉じ`**`の直後が仮名」「開き`**`の直前が仮名かつ直後が`[`」とは別の第3のパターン。
- **原因**: CommonMarkのemphasis規則は「直前・直後が Unicode punctuation/symbol カテゴリか」で判定するが、`$` は Unicode の Sc（通貨記号）カテゴリに属し、CommonMarkの punctuation 判定に含まれる。したがって開き`**`の直後が`$`だと、`isPunct(next)=true` かつ `isSpace(prev)`/`isPunct(prev)` がどちらも false（直前が地の文の仮名など）の場合、left-flanking条件を満たさず開きとして機能しない。#78のスキャナスクリプトの `isPunct` 正規表現（`!-\/` の範囲）は元々 `$`（ASCII 0x24）を含んでいたので、このスクリプトを流せば検出できたが、実装時に見落としていた。
- **対策**: 太字にしたい語句が直後にインライン数式 `$...$` を伴う場合、太字の範囲から数式を外す（数式部分は太字にしない）か、`**` と `$` の間に地の文を挟む。迷ったらそもそも「太字＋数式」の組み合わせを避け、数式側の強調は`\htmlId`のハイライト色や別の言い回しに任せる方が安全。
- **判断の目安**: #78のスキャナスクリプトの `isPunct` 正規表現は `$` や `#`・`%` など元々ASCII記号を広く含んでいるため、太字が数式・コード片（バッククォート）・記号列に隣接する新規コンテンツでは、スキャナを流すだけでなく「太字の境界がプレーンな仮名/漢字/英数字以外に接していないか」を目視でも確認する。

## Playwright実機確認でKaTeX項の値を読むとき、`textContent`は3重に連結される——`.katex-html`にスコープして読む（出典 #88）

- **症状**: `document.querySelector('#term-aA').textContent` で数式項の値を読むと、値が"9"のはずが"999"のように3回連結されて見え、一瞬「同じidの要素が3つある」ように誤解した。
...
## MDX太字が閉じ`**`の右フランキング規則で消える（続報その2）：`**<Term id="...">`のように地の文の仮名の直後に直接続くと崩れる——ただし既存の大量のトピックMDXも同じ書き方をしている（出典 #90）

- **症状**: 情報理論トピックで「…重み付けた平均（期待値）が**<Term id="entropy">エントロピー</Term>** $H(X)$」のように、太字の開始直後に間を置かず`<Term id="...">`が続く書き方で、太字が効かず`**`がそのまま画面表示された。Playwrightで`document.body.innerText.indexOf('**')`を使い機械的に検出した。#73/#78/#88で見つけたパターン（閉じ`**`直後が仮名／開き`**`直前が仮名かつ直後が`[`／開き`**`直後が`$`）とは別の第4のパターンだが、原因はすべて同じCommonMarkのflanking規則。
- **原因**: `<`はCommonMarkのpunctuation判定（ASCII記号）に含まれるため、開き`**`の直後が`<Term`（`<`で始まる）だと、直前が地の文の仮名（句読点でも空白でもない）の場合はleft-flanking条件を満たさず開きとして機能しない。ただし**ソース上の行頭に`**<Term`がある場合は、直前の改行が空白として扱われるため、この問題は起きない**（`prev=space`でleft-flanking条件を満たす）——実際、`grep -rn '\*\*<Term' content/topics/*.mdx`で検索すると数十件ヒットしたが、目視確認した範囲では**行頭に`**`が来ている箇所は正しく太字になっており、地の文の途中（仮名の直後）に`**<Term`が来ている箇所だけが壊れていた**。既存トピックの大半は前者（行頭）のパターンだったため、このバグは#90まで顕在化しなかったと推測される。
- **対策**: `**<Term ...>`の直前が地の文の仮名・漢字で終わる場合は、`**`の前に半角スペースを1つ挟む（例: 「が**<Term...` → 「が **<Term...`）。#78の対策（開き側は直前を空白/句読点にする）と同じ考え方——閉じ`</Term>**`側は`Term`の閉じタグ`>`自体がpunctuationなので、後ろに句読点や空白が続けば通常は問題にならない。
- **判断の目安**: 新規トピックMDXは、Playwrightで`document.body.innerText.indexOf('**') !== -1`を使い機械的に「**」の残存を検出する（目視だけでは行頭/行中の違いに気づきにくい）。検出したら該当箇所が「地の文の仮名 + `**` + 記号（`<`/`[`/`$`等）」の並びになっていないか確認し、半角スペースを挟んで直す。既存トピックに同型の(行中の)バグが潜んでいる可能性があるが、本Issueのスコープ外（自分が新規に書いたファイルのみ対象）として今回は対応していない——サイト全体の太字崩れ監査は別Issueが望ましい。

## 既存の公開済みMDXに`prettier --write`をかけると、地の文と`<Term>`/JSXタグの間に意図しない空行(段落分割)が挿入されることがある（出典 #92）

- **症状**: value-of-information.mdx・decision-analysis.mdxの前提関係リンクを更新した後、変更ファイルを`prettier --write`で整形したところ、`git diff`に「行の内容は変わっていないのに`<Term id="...">`や`<EVPI>`のようなJSXタグで始まる行の直前に空行が追加される」という差分が複数箇所現れた。MDXでは空行が段落の区切りとして扱われるため、この自動整形をそのままコミットすると、地の文が「不自然な位置で改行された別々の段落」としてレンダリングされ、視覚的な文章の流れが崩れる（今回は事前にPlaywrightで比較検証したため気づけたが、目視のdiffレビューだけでは見落としやすい）。
- **原因**: `prettier-plugin-tailwindcss`等を含む現在のPrettier設定が、MDXのインライン要素（地の文中に直接書かれた`<Term>`のような行頭JSXタグ）を「新しいブロック要素の開始」と誤認識し、前に空行を挿入する既知の挙動。行末が句読点や`・`のような記号で終わり、次の行が`<`で始まる箇所で特に起きやすい。
- **対策**: **新規作成したMDXファイル**（今回のutility-theory.mdxや新規term等）は最初から`prettier --write`をかけた状態で執筆・確認しているため問題ないが、**既存の公開済みMDXファイルを軽微に編集する場合は、そのファイル全体に`prettier --write`をかけない**——変更した行だけを`Edit`ツールでピンポイントに書き換え、ファイル全体の整形はスキップする。`pnpm format:check`がその既存ファイルを警告してもよい（この既存挙動はリポジトリ全体に既に273ファイル分存在する状態であり、個別PRで一括整形しようとするとかえって新しい壊れ方を生む）。
- **判断の目安**: 既存の`status: published`なMDXファイルを編集した後は、`git diff`で変更行数が「自分が意図した変更」だけに収まっているかを確認する。予期しない空行の追加・削除が混じっていたら、`git checkout -- <file>`で一度戻し、`Edit`ツールで該当箇所だけを再度書き換える。整形の要否と内容の正しさを混同しない——`prettier --write`は「常に安全」ではなく、MDXでは意味を変えうる。

## MDX太字が閉じ`**`の右フランキング規則で消える（続報その3）：`**カーディナル(基数的)**な尺度`のように、閉じ`**`の直前が`)`等の記号で直後が仮名だと崩れる（出典 #92）

- **症状**: 効用理論トピック・vNMの定理の用語ページで「この $u$ は**カーディナル(基数的)**な尺度」のように、太字の閉じ`**`の直前が`)`（丸括弧閉じ）、直後が「な」（仮名）という並びで太字が効かず`**`がそのまま表示された。Playwrightで`document.body.innerText.indexOf('**')`を全`<details>`展開状態で機械的に検出した。#73/#78/#88/#90で見つけた4パターンとは異なる第5のパターンだが、原因はやはり同じCommonMarkのflanking規則（右フランキング側）。
- **原因**: 閉じ`**`が右フランキング（emphasisを閉じられる）と判定されるには「直前が空白でない」に加え「(a)直前が punctuation でない、または (b) 直前が punctuation かつ直後が空白/punctuation」のいずれかを満たす必要がある。`カーディナル(基数的)**な尺度`では閉じ`**`の直前が`)`（punctuation）で、直後が「な」（仮名、空白でもpunctuationでもない）——(a)も(b)も満たさず右フランキングにならない。#90で見つけた「開き`**`直後が`<`」とは逆に、こちらは**閉じ側**が`)`のような通常の半角記号に隣接するケースで、数式や用語リンクを含まない普通の日本語の地の文でも起こりうる。
- **対策**: 太字にする語句の直前直後に丸括弧付きの補足（`(基数的)`等）を含める場合は、閉じ`**`の前に半角スペースを1つ挟む（例: 「カーディナル(基数的)**な」→「カーディナル(基数的) **な」）。あるいは補足の丸括弧を太字の外に出す（`**カーディナル**(基数的)な尺度`のように、太字範囲を丸括弧の前で終える）方が自然な場合も多い。
- **判断の目安**: Playwrightでの機械的な`**`残存チェックは、`)`・`」`・`』`のような閉じ括弧類が太字の境界（特に閉じ`**`の直前）に来ている箇所を重点的に見る。既存の4パターン（#73/#78/#88/#90）のチェックリストに「閉じ`**`の直前が閉じ括弧類で直後が仮名」を追加する。

## MDXの`|..|`パイプテーブル記法が`remark-gfm`未導入のため`<table>`にならず、ただの段落として描画される既存バグ（出典 #92）

- **症状**: 効用理論トピックでvNM公理を`| 公理 | 内容 |`形式のMarkdownテーブルで書いたところ、ブラウザで見ると罫線のない1つの段落としてパイプ文字がそのまま表示され、`document.querySelectorAll('table').length`が期待より少なかった。`grep -rl '^|.*|.*|$' content/topics/*.mdx content/terms/*.mdx`で調べると、`confusion-matrix.mdx`・`inclusion-exclusion-principle.mdx`・`logic-operation.mdx`・`partial-pooling.mdx`など既存の複数ファイルも同じ記法を使っており、これらも同じバグの影響を受けていたと判明した（本トピック固有ではなく、content/全体に既に存在していた潜在バグ）。
- **原因**: `next.config.mjs`の`remarkPlugins`が`[remarkFrontmatter, remarkMath]`のみで、GitHub Flavored Markdown拡張（テーブル記法を含む）を有効化する`remark-gfm`が入っていなかった。標準のCommonMark（remarkの既定）には`|..|`のパイプテーブル構文が無いため、パーサはそれを単なるテキスト（段落）として扱う。`app/topics/[slug]/page.tsx`・`app/terms/[slug]/page.tsx`はどちらも`import(...mdx)`で同じ`@next/mdx`（=同じ`next.config.mjs`設定）を経由するため、トピックMDX・用語MDXの両方に影響する。
- **対策**: `pnpm add remark-gfm`でインストールし、`next.config.mjs`の`remarkPlugins`に`remarkGfm`を`remarkFrontmatter`の直後に追加した。あわせて`mdx-components.tsx`に`table`（`overflow-x-auto`で包む）・`thead`・`th`・`td`のデフォルトスタイルを追加し、テーブルがモバイル幅で罫線付き・横スクロール可能に描画されるようにした（後述のモバイル横スクロール教訓とも関連）。
- **判断の目安**: 新規トピック・用語でMarkdownテーブル（`| .. | .. |`）を使う場合は、まずPlaywrightで`document.querySelectorAll('table').length`が期待件数と一致するか確認する（0件やレイアウト崩れなら`remark-gfm`の有無を疑う）。この修正は`next.config.mjs`という共有設定への変更なので、既存トピック・用語ページの表示に影響が及ばないか（悪化ではなく修正のみである）を`pnpm build`の全ページ生成成功と既存Vitestの全件パスで確認してからコミットする。

## モバイル幅でKaTeXの1行の`$$displayMath$$`がページ全体を横スクロールさせる——`.katex-mathml`の既知の軽微な問題（#84）とは別に、見た目にも数式が画面外に切れる実バグがありうる（出典 #92）

- **症状**: 効用理論トピックのL0で`E[X]=\underbrace{...}_{n=1}+\underbrace{...}_{n=2}+\underbrace{...}_{n=3}+\cdots=1+1+1+\cdots=\infty`のような長い1行の`<Math tex="..." />`（display既定=true）を書いたところ、モバイル幅390pxで実際にスクリーンショット上も数式が画面右端で切れ、`document.documentElement.scrollWidth`が515px（差125px）まで増えた。#84の教訓「`.katex-mathml`（アクセシビリティ用の隠し要素）起因でscrollWidthは増えるが視覚的な崩れは無い」は既知のスコープ外事項だが、今回は**視覚的にも実際に数式がはみ出して見える**という別種の問題だった。
...
- **症状**: 効用理論トピックのUtilityDecisionLabで、CARA効用（`Math.exp`を使う）から計算した棒グラフの幅を`style={{ width: `${(Math.abs(v)/max)*100}%` }}`のように未加工の浮動小数点数のまま埋め込んだところ、初回ロード時にReactのhydration mismatchエラー（`width: "49.07102573379076%"`(クライアント) vs `width: "49.071%"`(サーバ)）が発生した。SVG座標の丸め忘れ（#67/71/74）と紛らわしいが、今回はSVGではなく通常のHTML要素の`style`属性だった。
...
## 1行に「=」で3〜4個の項を連結した長い`$$...$$`（連立導出）は`\boxed{}`と無関係にモバイルで実際に画面外へはみ出す（出典 #95）

- **症状**: ニューラルネットワークの仕組みトピックのL1「シグモイドの微分」導出で、`$$\sigma'(z)=\frac{(1+e^{-z})-1}{(1+e^{-z})^2}=\frac{1}{1+e^{-z}}-\frac{1}{(1+e^{-z})^2}=\sigma(z)-\sigma(z)^2=\sigma(z)\bigl(1-\sigma(z)\bigr)$$`のように、1行に`=`で4つの分数項を横に並べた式を書いたところ、モバイル幅390pxで`document.documentElement.scrollWidth`が623px（差233px、`window.scrollTo`で実際に233px横スクロールできる）になった。同じページの直後にある短い`$$\boxed{\ \sigma'(z)=\sigma(z)(1-\sigma(z))\ }$$`（結論だけの短い式）も一見疑わしく見えたが、既存の公開済みトピック（learning-framework.mdxの同型`\boxed{...}`）で同じチェックをすると差はわずか49px（#84の許容範囲）だったため、`\boxed`自体は無罪で、原因は**その直前の長い連立の`$$...$$`**と判明した。
- **原因**: #92で見つけた「KaTeXのdisplayモード数式は折り返されない」という制約は、`\underbrace`のような装飾要素だけでなく、**単に項の数が多い1行の連立等式**でも同じように起きる。項を`=`で3つ以上つなぐと、各項の分数（`\frac{...}{...}`）の幅が横に積み上がり、フォーマットとしては「1つの式」でも見た目の総幅は非常に広くなる。
- **対策**: 導出の中間式で「同じ量を表す複数の書き換えを`=`で並べる」場合は、最初から`\begin{aligned}\sigma'(z)&=\text{式1}\\&=\text{式2}\\&=\text{式3}\end{aligned}`のように**各等号の前で改行**し、`&=`で位置を揃える（`<Math>`/`$$...$$`は`aligned`環境をそのまま受け付ける、#92で確立済みのパターン）。「項が2つまでの短い等式」は許容範囲のことが多いが、**3つ以上の項を`=`で連結する式を書いたら、執筆時点でモバイル確認を待たずに`aligned`化しておく**のが安全。
- **判断の目安**: モバイル確認で`scrollWidth`超過が見つかったら、`\boxed`や`\underbrace`のような装飾コマンドをまず疑いがちだが、**同じ装飾コマンドが既存の公開済みトピックで既に使われていて差が小さい（#84許容範囲）なら、疑うべきは近傍の別の数式**——特に「1行に`=`で3項以上つないだ連立式」を機械的に探す（`grep -n "=.*=.*="`でMDX中の`$$...$$`ブロックを洗い出すと見つけやすい）。

## MDX本文の生の`$$...$$`（自前のoverflow-x-autoラッパーを持たない）は、短く見える数式でも`\text{日本語}`注釈を含むとモバイルで実際にはみ出す（出典 #96）
- **症状**: NNモデル（CNN・RNN）トピックのL0で、`$$z_{i,j}=\sum...+b \qquad(\text{CNN: 畳み込み層})$$`のように、数式の末尾に`\qquad(\text{...})`で短い日本語ラベルを添えたところ、式自体は短いのにモバイル幅390pxで`document.documentElement.scrollWidth`差が64px（`.katex-mathml`を除外した可視要素のオーバーフローで確認、#84の既知の軽微な問題ではない）まで増えた。`<MathFormula>`コンポーネント経由の数式は自前で`overflow-x-auto`のdivに包んでいたため無事だったが、トピックMDX本文に直接書いた`$$...$$`（remark-math/rehype-katex がそのままレンダリング）にはそのラッパーが無く、はみ出しがページ全体に影響した。
- **原因**: `\text{}`内の日本語は`cjk_fallback`というフォールバックフォントでレンダリングされ、思ったより横幅を取る。加えて`mdx-components.tsx`は素の`$$...$$`由来のKaTeX出力を`overflow-x-auto`で包んでいない（テーブルや`<MathFormula>`は個別に対応済みだが、地の文の`$$...$$`は対象外）。短い数式だから安全、という判断は「文中に`\text{日本語}`が入っていない場合」に限られる。
- **対策**: 地の文で既に同じ内容を説明している（例: 直前・直後の`<Term>`付き文章で「CNN」「RNN」に言及している）場合は、数式内の`\qquad(\text{...})`のような重複ラベルを削除する（#92の対策方針の具体例）。どうしても数式内に注釈を残したい場合は、その`$$...$$`をトピック本文用の`overflow-x-auto`ラッパー（`<MathFormula display>`を使うか、mdx-components.tsx側で素の`$$...$$`もラップするサイト全体の改修）で包む。
- **判断の目安**: 新規トピックのモバイル確認では、「式が短いから大丈夫」と判断せず、`\text{}`（特に日本語を含むもの）を数式内に持つ`$$...$$`ブロックを`grep -n '\\text{'`で機械的に洗い出し、1つずつ`el.closest('.katex-mathml')`と`el.closest('.overflow-x-auto')`を除外した可視要素オーバーフロー判定（#94の方法）にかける。トピックMDX本文の裸の`$$...$$`は`<MathFormula>`と違って自動でoverflow-x-autoに包まれないことを常に意識する。

## `<select>`要素の閉じたボックス幅は、明示的に幅を指定しない限り最長の`<option>`テキストに引っ張られてモバイルで横スクロールする（出典 #97）

- **症状**: テキスト解析トピックのPreprocessStepper/TfidfLabで、文書を選ぶ`<select>`の`<option>`文言に`D1: data science combines statistics and programming`のような長い英文全体を入れていたところ、モバイル幅390pxで`document.documentElement.scrollWidth`が483px（差93px）になった。`el.closest('.katex-mathml')`/`el.closest('.overflow-x-auto')`を除外した可視要素オーバーフロー判定（#94の方法）で犯人を特定したところ、KaTeX関連ではなく`<select id="ta-preprocess-sentence">`自体の`getBoundingClientRect().right`が484pxまで達しており、`<select>`要素そのものがはみ出しの原因だった。
- **原因**: `<select>`に明示的な`width`/`max-width`を指定しない場合、Chromiumは閉じた状態のボックス幅を**最長の`<option>`テキストに合わせて自動的に広げる**（`<input>`の既定幅とは挙動が異なる）。Tailwindの`className="rounded-lg border ... text-sm"`のように装飾クラスだけを与え幅を制約していないと、親要素が`flex flex-wrap`で折り返し可能でも`<select>`自身の内在幅（intrinsic width）がそれを上回り、ページ全体を横スクロール可能にしてしまう。
- **対策（今回の判断）**: `<option>`文言を`D1`のような短いラベルに変え、選んだ文書の全文は`<select>`の隣に`<span className="min-w-0 flex-1 break-words text-xs text-slate-500">{fullText}</span>`として別途表示する形に変更した。**`min-w-0`を付け忘れると、flexアイテムの既定値`min-width:auto`により長いテキストが折り返されず同じ横スクロールが再発する**（flexアイテムは自身のコンテンツ幅より縮まないのが既定挙動）ので、`flex-wrap`コンテナ内に長いテキストを流し込む`<span>`には常に`min-w-0`（または`break-words`と併用）を付ける。
- **判断の目安**: 新規トピックで`<select>`の`<option>`に「文全体」「長い説明文」を入れる設計をしたら、執筆時点でモバイル確認を待たずに`<option>`は短い識別子（D1・A・①等）にとどめ、全文表示が必要ならセレクトの外側（別要素）に出す。モバイル確認で犯人不明のはみ出しが見つかったら、`.katex-mathml`/`.overflow-x-auto`除外に加えて`<select>`・`<input>`など**フォームコントロール自体**も候補に入れて`getBoundingClientRect()`を機械的に洗い出す。

## 用語ページ（`/terms/[slug]`）もトピックMDX本文と同じく、bare`$$...$$`が自前のoverflow-x-autoラッパーを持たずモバイルで横スクロールしうる（出典 #97）

- **症状**: テキスト解析Issueで新規作成したtf-idf.mdx（2つの計算例`\text{idf(data)}=\ln(5/4)\approx0.223,\qquad \text{idf(combine)}=\ln(5/1)\approx1.609`を1行にqquad連結）・n-gram.mdx（3つ組のタプル`(\text{data},\text{science}),\ (\text{science},\text{combines}),\ (\text{combines},\text{statistics})`を1行に連結）・language-model.mdx（`P(w_i\mid ...)\approx P(w_i\mid ...)`の2段等式）・vector-space-model.mdx（$m$項の省略記法ベクトル）の4ファイルで、モバイル390pxのscrollWidth差がそれぞれ175px/115px/28px/51pxに達した。
- **原因**: `app/terms/[slug]/page.tsx`は`<main className="mx-auto max-w-2xl px-6 py-12">`で幅を制約しているだけで、トピックページの`mdx-components.tsx`同様に素の`$$...$$`由来のKaTeX出力をoverflow-x-autoで包んでいない（#96で判明したトピックページの問題と全く同じ構造が用語ページにもある）。既存の40件超の公開済み用語ページ（backpropagation.mdx等）にも同種の長い`\text{日本語}`パターンが多数残存していることを`grep -nP '\\text\{[^}]*[^\x00-\x7F]' content/terms/*.mdx`で確認したが、#84の既存判断（サイト横断のKaTeX/CSS問題はIssue単位のスコープ外）を踏襲し、**自分が新規作成したファイルのみ**その場で対処した。
- **対策**: #92/#95で確立した「連立等式は`\begin{aligned}`または複数の`$$...$$`に分割」「1行に3項以上を連結しない」の方針を、用語ページの新規作成時にも適用する。具体的には「2つの計算例を1行にqquad連結」→2本の`$$...$$`に分割、「3つ組を1行に連結」→3本の`$$...$$`に分割、「2段の`\approx`等式」→2本に分割。
- **判断の目安**: 新規に用語ノード（`content/terms/*.mdx`）を書くときも、トピックMDX本文と同じ横スクロールチェック（`document.documentElement.scrollWidth`をモバイル390pxで比較、`.katex-mathml`/`.overflow-x-auto`除外の可視要素オーバーフロー判定）を`/terms/<slug>`に対しても行う。「用語ページは短いから大丈夫」と判断せず、特に「複数の具体例をqquadで1行に並べる」「タプル・ペアを複数個1行に並べる」書き方をしたら執筆時点で分割しておく。

## モバイル横スクロールの既知パターン（#95/#96）を「意識して書いた」だけでは再発を防げない——複数Derivationを書くLevelでは必ず実測で全数チェックする（出典 #98）

- **症状**: 画像解析トピックのL1「IoUの分母（和集合の面積）」Derivationで`Area(A\cup B)=Area(A)+Area(B)-Area(A\cap B)`（3項を`=`/`-`で連結、#95で既知のパターン）を書いてしまい、モバイル390pxで`document.documentElement.scrollWidth`差26pxを検出した。同じLevel内の別のDerivation（ソフトマックス導出）にも`e^{z_1},\ e^{z_2},\ \dots,\ e^{z_K}\quad(\text{すべて正})`という`\text{日本語}`注釈（#96で既知のパターン）が紛れ込んでいた。さらにL2のアンカーボックス連立方程式でも`\qquad(\text{面積の目安})`という同型の日本語注釈があった——**同じセッション内で#95/#96の教訓を読み返しながら執筆したにもかかわらず**、3箇所も紛れ込んだ。加えて`content/terms/intersection-over-union.mdx`にも同じ`Area(A∪B)=...`の3項連結式をコピー的に書いてしまい、term側でも同じ修正が必要になった。
- **原因**: 導出（Derivation）を書くとき、「結論の数式を分かりやすく1行にまとめたい」「地の文の説明を数式側にも軽く添えたい」という執筆時の自然な衝動が、既知のはみ出しパターンを何度でも再生産する。既知パターンを列挙したチェックリストを意識していても、新しい文章を書く集中の中では見落とす——「知っている」ことと「書いている最中に適用できる」ことは別。
- **対策**: 執筆中に既知パターンを避けようとするだけでなく、**実装完了後に必ずPlaywrightで`scrollWidth`の実測を行い、差が0でなければ`el.closest('.katex-mathml')`/`.overflow-x-auto`を除外した可視要素の中から犯人の`.katex`要素を`textContent`で特定して機械的に潰す**（本教訓の実例: `span.base`の`getBoundingClientRect().right > clientWidth`な要素を`closest('.katex')`まで遡り`textContent`を読むと、はみ出している数式の中身が一発で分かる）。同じ数式パターンをterm側にも書いていないか`grep`で横断確認する。
- **判断の目安**: 複数のDerivationブロックを持つLevelを実装したら、「既知パターンを避けて書いたから大丈夫」で済ませず、必ず実機の`scrollWidth`計測を行う。差が見つかったら、まず`span.base`（KaTeXの可視要素）を`closest('.katex')`まで遡って`textContent`を読み、どの数式が原因か特定してから直す——目視でのDerivation再読より速く確実。

## Playwrightで長いページのSVGドラッグ操作を検証するとき、`scrollIntoViewIfNeeded()`を挟まないと座標がヒットせず「操作しても何も起きない」ように見える（出典 #98）

- **症状**: 画像解析トピックのIoULab（2つのバウンディングボックスをSVGドラッグで動かすラボ）をPlaywrightで検証する際、`element.getBoundingClientRect()`で取得した中心座標へ`page.mouse.move/down/move/up`を発行してもIoUの値が全く変化しなかった。ドラッグ先のrect自体に`pointerdown`リスナーを一時的に付けて発火を確認したところ`fired: false`——クリックが対象要素に届いていなかった。
- **原因**: `getBoundingClientRect()`は常に「現在のビューポート」基準の座標を返す。ページが長く（本トピックはLevelを跨ぐ長尺ページ）、対象要素が現在のスクロール位置からビューポート高さを超えて離れていると、返ってくる`top`/`y`がビューポート高さ（例900px）を大きく超える値（例2771px）になる——このような画面外の座標へ`page.mouse.move`しても、実際のブラウザウィンドウ上にはその座標が存在しないため、どの要素にもヒットしない（クリックしても無反応）。`page.locator(...).click()`のような高レベルAPIは自動でスクロールするが、`page.mouse.move/down/up`の低レベルAPIは自動スクロールしない。
- **対策**: 低レベルの`page.mouse`でドラッグ等を検証する前に、必ず対象要素に対して`await element.scrollIntoViewIfNeeded()`（またはPlaywrightの`locator.scrollIntoViewIfNeeded()`）を呼んでから`boundingBox()`/`getBoundingClientRect()`を取得する。
- **判断の目安**: `page.mouse`でクリック・ドラッグしたのに状態が変化しない場合、まず「対象要素が現在のビューポート内に実際に見えているか」（`scrollIntoViewIfNeeded`を挟んだか）を疑う。次に、同じ操作を「一時的なイベントリスナーを付けて発火の有無だけ確認する」という切り分けで検証すると、座標の問題なのか実装側のバグなのかを素早く判別できる。

## 重なり合う複数のSVGヒットエリアは、クリック座標が境界線上だと後から描画された要素が意図せず勝つ（出典 #98）

- **症状**: IoULabで「ボックスAの中心」のつもりで選んだクリック座標（canvas座標(35,35)）が、実は初期配置のボックスB（x:35-75, y:30-70）の境界線ちょうど上にも該当しており、DOM順で後に描画された（＝手前にある）ボックスBの透明なドラッグ用hitエリアが先にヒットしてしまい、ボックスAが動かなかった（実際には狙っていない方の要素が反応していた）。
- **原因**: SVGは同じ位置に複数の要素が重なる場合、DOM順で後（＝描画順で手前）の要素がヒットテストを制する。座標を「見た目のボックスの中心」で選んだつもりでも、隣接・重なり合う別のボックスの領域にちょうど入り込んでいると、狙った要素ではなく手前の要素が反応する。
- **対策**: 複数の重なり得るSVG要素をクリック・ドラッグで区別するテストを書くときは、対象が**単独で占有している**（他の要素の領域と重ならない）座標を明示的に選ぶ。境界に近い・重なり領域内の座標は避ける。
- **判断の目安**: ドラッグ操作の実機テストで意図した要素が反応しない場合、選んだクリック座標が他の重なる要素の領域とも重複していないか確認する。座標だけで判断せず、`rects[].getAttribute('x'/'y'/'width'/'height')`で実際の各要素の占有範囲を洗い出してから、単独領域内の座標を選び直す。
