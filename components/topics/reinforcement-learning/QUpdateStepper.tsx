...
const FORMULA = `\\text{TD目標: } ${term("r", "?")} + ${formatNumber(Q_STEPPER_GAMMA, 2)}\\cdot${term(
  "maxq",
  "?",
)} = ${term("target", "?")} \\qquad \\text{TD誤差: } ${term("target2", "?")} - ${term("qsa", "?")} = ${term(
  "error",
  "?",
)}`;

const FORMULA2 = `Q(s,a)\\leftarrow ${term("qsa2", "?")} + ${formatNumber(Q_STEPPER_ALPHA, 2)}\\cdot${term(
  "error2",
  "?",
)} = ${term("qnew", "?")}`;
...
export function QUpdateStepper() {
  ...
  useEffect(() => {
    ...
    m.setValue("r", formatNumber(u.reward, 2));
    m.setValue("maxq", formatNumber(u.maxNextQ, 2));
    m.setValue("target", formatNumber(u.tdTarget, 2));
    m.setValue("target2", formatNumber(u.tdTarget, 2));
    m.setValue("qsa", formatNumber(u.qBefore, 2));
    m.setValue("qsa2", formatNumber(u.qBefore, 2));
    m.setValue("error", formatNumber(u.tdError, 2));
    m.setValue("error2", formatNumber(u.tdError, 2));
    m.setValue("qnew", formatNumber(u.qAfter, 2));
    m.setHighlight("r", true, "#059669");
    m.setHighlight("maxq", true, "#7c3aed");
    m.setHighlight("target", true, "#0f172a");
    m.setHighlight("target2", true, "#0f172a");
    m.setHighlight("qsa", true, tone);
    m.setHighlight("qsa2", true, tone);
    m.setHighlight("error", true, "#b91c1c");
    m.setHighlight("error2", true, "#b91c1c");
        ...
        <>
          <div className="overflow-x-auto">
            <svg viewBox={`0 0 ${W} ${H}`} className="mx-auto w-full max-w-xs" role="img" aria-label="現在のステップの状態遷移">
              ...
              <line
                x1={cellCenterX(cellFromIndex(p.step.state).col)}
                y1={cellCenterY(cellFromIndex(p.step.state).row)}
                x2={cellCenterX(cellFromIndex(p.step.nextState).col)}
                y2={cellCenterY(cellFromIndex(p.step.nextState).row)}
                stroke={explored ? "#d97706" : "#0f172a"}
                strokeWidth={2}
              />
              <circle
                cx={cellCenterX(cellFromIndex(p.step.state).col)}
                cy={cellCenterY(cellFromIndex(p.step.state).row)}
                r={7}
                fill={explored ? "#d97706" : "#0f172a"}
                stroke="#fff"
                strokeWidth={1.5}
              />
            </svg>
          ...
          <div className="overflow-x-auto rounded-xl bg-slate-50 px-4 py-3 text-center">
            ...
            <div className="mt-2">
              <MathFormula tex={FORMULA2} display={false} />
            </div>
          </div>
